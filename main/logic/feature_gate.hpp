#pragma once
// WHICH DERIVED FEATURES MAY HONESTLY RUN on the detected model — the policy half of issue legacy-69
// step 0.2 (spun off as legacy-110 Part C): what UC5 ("silent protection retries" early warning) and, later,
...
//     (legacy-121); when the outdoor unit stops refreshing its own pages the pills blank rather than
...
//
//  2. A REDUCED MODEL IS A DIFFERENT MODEL. UC5's thresholds — and anything trained in legacy-69 Phase 2 —
...
//     features" with extra steps, which is the one outcome legacy-69 rules out by name.
//
...
// legacy-69 frames this as a `generic`-fallback problem, and `generic` IS the extreme case: measured on
...
//
// Pure and host-tested for the usual reason: there is no firmware caller yet (legacy-69 Phase 3 has not
