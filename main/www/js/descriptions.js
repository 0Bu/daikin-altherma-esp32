...
const DESCRIPTIONS = [
  ...
  { re: /2nd domestic hot water|domestic hot water temperature (?:upper|lower)/i,
    what: "The reading from a temperature sensor in the hot-water tank, for example in a tank configuration with dual sensors at the top and bottom.",
    de: { what: "Der Messwert eines Temperaturfühlers im Warmwasserspeicher, zum Beispiel bei einem Speicher mit Fühlern oben und unten." } },
  ...
  { re: /tank preheat/i,
    what: "The X10A controller is preheating the tank before a configured demand or schedule. This is not the HomeHub's direct disinfection-operation flag and does not by itself prove that disinfection is running.",
    normal: "ON only during the configured preheating period; OFF otherwise. Compare its independent timeline with HomeHub input 33 when investigating disinfection.",
    de: { what: "Der X10A-Regler heizt den Speicher vor einem eingestellten Bedarf oder Zeitplan vor. Das ist nicht das direkte Desinfektionssignal des HomeHub und belegt allein keine laufende Desinfektion.",
          normal: "nur während der eingestellten Vorheizzeit ON, sonst OFF. Zur Beurteilung einer Desinfektion den unabhängigen Verlauf mit HomeHub-Eingang 33 vergleichen." } },
  { re: /^disinfection operation$/i, exact: true,
    what: "HomeHub input 33 reports whether the controller's tank-disinfection operation is active. It is a current operation flag, not the configured weekday, start time or temperature.",
    normal: "ON while disinfection is operating; OFF otherwise. The five-minute event timeline retains any ON observed during a bucket, but a pulse entirely between Modbus full polls can still be missed.",
    de: { what: "HomeHub-Eingang 33 meldet, ob die Speicherdesinfektion des Reglers aktiv ist. Das ist ein aktuelles Betriebssignal, nicht der konfigurierte Wochentag, die Startzeit oder die Temperatur.",
          normal: "während der Desinfektion ON, sonst OFF. Der Fünf-Minuten-Ereignisverlauf hält jedes im Zeitfenster beobachtete ON fest; ein vollständig zwischen zwei vollständigen Modbus-Abfragen liegender Impuls kann dennoch unbemerkt bleiben." } },

  // ── X10A control and actuator diagnostics ──
  // These are controller flags, not physical feedback contacts. Neutral ON/OFF wording is
  // deliberate until the new series have been correlated with live operating edges.
  { exact: true, group: "outdoor_state", re: /^thermostat on\/off$/i,
    what: "The outdoor controller's own thermostat-state flag. It is separate from the indoor unit's identically named thermo-demand bit and does not by itself prove compressor operation.",
  ...
  { exact: true, reg: 0x60, re: /^solar input$/i,
    ...
    de: { what: "Hydraulik-Bit Solar input; Funktion und Polarität sind nicht belegt.",
          normal: "ON heißt gesetzt. Mit Einstellungen und Kontaktwechseln abgleichen." } },
  { exact: true, re: /^(?:restart standby|startup control)$/i,
    what: "A controller phase flag for restart standby or startup control. It identifies internal sequencing, not useful heat delivery.",
    normal: "Short ON phases around starts can be normal. Compare their order with compressor frequency and faults.",
    de: { what: "Ein Regler-Phasenbit für Wiederanlauf-Bereitschaft oder Startsteuerung. Es kennzeichnet eine interne Ablaufphase und keine nutzbare Wärmeabgabe.",
          normal: "Kurze ON-Phasen rund um Starts können normal sein. Reihenfolge mit Verdichterfrequenz und Störungen vergleichen." } },
  { exact: true, re: /^oil return operation$/i,
    what: "The outdoor controller reports an oil-return operation intended to return refrigerant oil to the compressor. It is an internal refrigerant-circuit phase.",
    normal: "Temporary ON can be normal; repeated or long phases need operating context.",
    de: { what: "Die Außeneinheit meldet einen Ölrückführungsbetrieb, der Kältemaschinenöl zum Verdichter zurückführen soll. Das ist eine interne Phase des Kältekreises.",
          normal: "Vorübergehendes ON kann normal sein; wiederholte oder lange Phasen benötigen Betriebskontext." } },
  { exact: true, re: /^pressure equalizing operation$/i,
    what: "The controller reports a pressure-equalising phase in the refrigerant circuit. It describes sequencing, not a measured pressure or confirmed valve position.",
    normal: "Judge short phases together with compressor state, 4-way-valve bit and pressure.",
    de: { what: "Der Regler meldet eine Druckausgleichsphase im Kältekreis. Der Wert ist weder eine Druckmessung noch eine bestätigte Ventilstellung.",
          normal: "Kurze Phasen gemeinsam mit Verdichterstatus, 4-Wege-Ventilbit und Druck bewerten." } },
  { exact: true, re: /^demand signal$/i,
    what: "A proprietary outdoor-controller demand flag. The catalog does not define which request or control layer asserts it.",
  ...
  { exact: true, re: /^floor loop shut off valve$/i,
    ...
    de: { what: "Das Reglerbit für ein optionales Absperrventil des Fußbodenheizkreises. Es ist ein logischer Befehls- oder Statuswert und weder Stellungsrückmeldung noch Durchflussmessung.",
          normal: "Bis zur bestätigten Polarität ON/OFF mit Hydraulik, Pumpe und Durchfluss vergleichen." } },
  { exact: true, re: /^system off \(on:system off\)$/i,
  ...
  { exact: true, re: /^add\. ext\. rt input (?:cool|heat)\.$/i,
    ...
    de: { what: "Der externe Raumthermostat-Eingang der Zusatzzone für Kühlen oder Heizen. Er meldet einen elektrischen Reglereingang und weder Raumtemperatur noch Verdichterbetrieb.",
          normal: "ON sollte bei entsprechend konfigurierter Regelungsart dem zugehörigen Thermostatkontakt folgen. Heiz- und Kühleingang auf widersprüchliche Anforderungen vergleichen." } },
  { exact: true, re: /^main rt (?:cooling|heating)$/i,
    what: "The main zone's room-thermostat request bit for Cooling or Heating. It is a demand input and not confirmation that the requested mode is being delivered.",
    normal: "Normally only the intended mode requests ON; confirm response with mode, pump, valve and compressor.",
    de: { what: "Das Raumthermostat-Anforderungsbit der Hauptzone für Kühlen oder Heizen. Es ist ein Anforderungseingang und keine Bestätigung, dass die angeforderte Betriebsart tatsächlich geliefert wird.",
          normal: "Normalerweise fordert nur die gewünschte Betriebsart ON; Reaktion mit Modus, Pumpe, Ventil und Verdichter bestätigen." } },
  { exact: true, re: /^pwr consumption limit [1-4]$/i,
    what: "One of four raw power-consumption-limit controller bits. Their combination is exposed unchanged because one-hot versus priority encoding has not yet been established on the live unit.",
    normal: "Graph all four bits separately and correlate them with compressor frequency and measured power. Do not derive a single limit stage until observed combinations establish the encoding.",
    de: { what: "Eines von vier rohen Reglerbits zur Leistungsbegrenzung. Ihre Kombination wird unverändert gezeigt, weil Einzelauswahl oder Prioritätskodierung an der realen Anlage noch nicht geklärt ist.",
          normal: "Alle vier Bits getrennt darstellen und mit Verdichterfrequenz sowie gemessener Leistung korrelieren. Erst nach beobachteten Bitkombinationen eine gemeinsame Begrenzungsstufe ableiten." } },
  { exact: true, re: /^phe heater$/i,
    what: "The controller's PHE-heater output/status bit. The catalog does not establish whether it is a command or feedback signal, so it does not by itself prove heater current.",
    normal: "Compare neutral ON/OFF with temperature, operating phase and measured power.",
    de: { what: "Das Ausgangs- oder Statusbit der PHE-Heizung. Der Katalog klärt nicht, ob es Befehl oder Rückmeldung ist; allein belegt es daher keinen Heizstrom.",
          normal: "Neutrales ON/OFF mit Temperatur, Betriebsphase und gemessener Leistung vergleichen." } },
  { re: /reheat on/i,
  ...
  { re: /mix valve position|bizone kit mix valve|(?:bypass|tank) valve position/i,
    what: "How far the modulating valve is open. Regulates between fully closed and fully open according to operating demand.",
    normal: "moves between fully closed and fully open as needed.",
    de: { what: "Wie weit das modulierende Ventil geöffnet ist. Regelt je nach Anforderung zwischen ganz geschlossen und ganz geöffnet.",
          normal: "regelt je nach Bedarf zwischen ganz geschlossen und ganz geöffnet." } },
  ...
  { exact: true, re: /^(?:smart[- ]grid operation mode|demand response mode)$/i,
    what: "The Smart-Grid / demand-response request: Free running, Forced off, Recommended on or Forced on. The HomeHub reports it directly; the X10A row derives the same four states from contacts 1 and 2. It is an energy-management command, not the outdoor unit's Heating/Cooling mode.",
    ...
    de: { what: "Die Smart-Grid-/Demand-Response-Anforderung: Freier Betrieb, Zwangsabschaltung, Empfehlung ein oder Erzwungen ein. Der HomeHub meldet sie direkt; die X10A-Zeile leitet dieselben vier Zustände aus Kontakt 1 und 2 ab. Das ist ein Energiemanagement-Befehl und nicht der Heiz- oder Kühlmodus der Außeneinheit.",
  ...
  { re: /discharge pipe|compressor outlet|inv discharge/i,
    ...
    normal: "Usually the hottest point in the circuit, well above the condensing temperature. It varies with pressure ratio, compressor speed, mode, load and outdoor conditions. Compare only stable runs of the same model: one value or a range from another product family proves neither a fault nor low refrigerant charge. A very high value can make the unit limit output to protect the compressor.",
    de: { what: "Temperatur des heißen, verdichteten Kältemittelgases, das den Verdichter verlässt.",
          normal: "Meist der heißeste Punkt im Kreislauf, deutlich über der Kondensationstemperatur. Der Wert hängt von Druckverhältnis, Verdichterdrehzahl, Betriebsart, Last und Außenbedingungen ab. Nur stabile Läufe desselben Modells vergleichen: Weder ein Einzelwert noch ein Bereich aus einer anderen Produktfamilie beweist Störung oder Kältemittelmangel. Ein sehr hoher Wert kann zur Schutz-Rückregelung führen." } },
  ...
  { re: /r4t-deicer/i,
    ...
    normal: "Sensor position and defrost control are model-specific. One local reading does not establish the whole coil's ice state, when defrost should start or why it ended.",
    de: { what: "Temperatur des Enteiser-Fühlers am Außenwärmetauscher. Die Außeneinheit verwendet sie als einen Eingang für die Entscheidung über Frostschutz und Abtauen.",
          normal: "Fühlerposition und Abtauregelung sind modellspezifisch. Ein lokaler Messwert belegt weder den Vereisungszustand der gesamten Fläche noch den richtigen Start oder den Grund für das Ende einer Abtauung." } },
  ...
  { re: /expansion valve|^ev \(/i,
    ...
    de: { what: "Reglerbefehl für ein elektronisches Expansionsventil, angegeben in Motorschritten oder Impulsen. Das ist keine unabhängige Rückmeldung, dass das Ventil die Stellung erreicht oder mechanisch hält, und weder Öffnungsprozentsatz noch direkter Massenstrom.",
          normal: "Der Befehl wird bei laufendem Verdichter nachgeregelt und kann im Stillstand oder beim Abtauen einen regelungsspezifischen Wert annehmen. Nur dasselbe Ventil bei gleichem Modell und gleicher Betriebsart vergleichen. Der Befehl allein belegt weder Bewegung noch klemmendes Ventil oder Kältemittelmangel." } },
  ...
  { re: /target (evap|cond)/i,
    ...
    normal: "The target is model- and mode-specific. Compare it with the matching pressure-derived saturation temperature during a stable run; a difference shows control error at that instant, not its cause or a refrigerant-charge diagnosis.",
    de: { what: "Eine interne Regelvorgabe für den Kältekreis. Ziel ist die Verdampfungs- oder Kondensationstemperatur; dieser Wert wird nicht vom Benutzer eingestellt.",
          normal: "Das Ziel ist modell- und betriebsartspezifisch. Im stabilen Lauf mit der passenden druckberechneten Sättigungstemperatur vergleichen; eine Abweichung zeigt die momentane Regeldifferenz, nicht deren Ursache oder eine Diagnose der Kältemittelfüllung." } },
