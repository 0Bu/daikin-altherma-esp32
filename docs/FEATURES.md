...
| 42 | **24-hour trend rings** — a fixed-cadence (5 min × 288) `int16` ring per trended value or state, **static** rather than on the heap (576 B each; **31 X10A/board/state rings = 17856 B** plus **twelve label-free HomeHub rings = 6912 B** and **three ENV III rings = 1728 B**, **26496 B total**, guarded by separate ceiling asserts). Their non-zero `pending` sentinel places the rings in `.data`, not `.bss`, so each also costs its size in the flash image. Chunked `GET /history` serves X10A by default, reports the configured circulation-power witness as MQTT, accepts `source=modbus` for HomeHub and `source=env3` for ENV III temperature/humidity/pressure; the UI draws the result in value-row explainers and schematic inspectors, with all three ENV measurements combined in one chart with unit-specific scales. Eight numeric pairings overlay blue X10A and petrol HomeHub lines on one aligned scale, including post-BUH R2T and refrigerant-liquid R3T. Nine categorical timelines cover Smart-Grid mode, the tank heater, BUH, the 3-way valve, the optional 2-way/heating-cooling output, the raw X10A water-flow-switch status, the external circulation pump, Quiet and Defrost. BSH and BUH event-fold an observed ON within the open bucket; persistent selector/output states retain the latest sample. The UI calls these sampled raster intervals, not exact runtime. There is no heater-specific power register: HomeHub input 51 remains the heat-pump power-consumption value reported by the register interface; its exact measurement boundary is not documented here. Outdoor-coil R4T, 2WV and water-flow-switch history are X10A-only; the circulation timeline is derived read-only from the confirmed MQTT active-power mapping and resets when that evidence identity changes. Two X10A absences remain distinguished: a register that did not answer vs. an outdoor-unit page held over while the compressor rests. Five displayed figures (pump speed, ΔT, heat output, electrical input, COP) plus the combined BUH state have no single ring matching the shown figure; `www/js/history.js` assembles their curves/timeline from source rings using the live expressions. Two primary trends are the board's own **free heap** and **largest contiguous block**. Numeric row trends are addressed structurally by **(register page, byte offset, unit)**, shared-bit trends add the converter, and Smart-Grid combines two structurally identified contacts. HomeHub history membership is explicit in `HOMEHUB_HISTORIES`; compile-time guards pin it to real trends and unique offsets | ✅ 🧪 | [`logic/history.hpp`](../main/logic/history.hpp), [`logic/homehub_map.hpp`](../main/logic/homehub_map.hpp), [`logic/env3.hpp`](../main/logic/env3.hpp), [`history.cpp`](../main/history.cpp), [`http_status.cpp`](../main/http_status.cpp), [`www/js/history.js`](../main/www/js/history.js) |
...
| 62 | **Configurable MQTT living-room source — canonical heating-curve-diagnosis input.** Settings stores one presentation name, one exact MQTT topic (wildcards refused), required paths for current temperature, target and source timestamp, optional enabled/HVAC-mode paths, and a 10–3600 s maximum age. A non-empty mapping is test-before-persist on the existing authenticated MQTT client and produces a mapping-bound, single-use proof only after bounded JSON parsing, plausibility and freshness pass. Saving the mapping is consent to subscribe; deleting it unsubscribes and clears captured runtime state while the independent Test path stays reachable before save. Retained timestamp-less, stale/future/backward, missing and out-of-range data fail closed. `/status.reference_temperature` and the grouped `room` object on `<base>/heating_curve` expose the accepted values, validity, stable reason/code and canonical `error_k = target - actual` only when every configured gate passes. Positive means too cold; negative means too warm. No MQTT command or heat-pump write exists. The UI ships no vendor preset; an ordinary mapping can describe Meross or any compatible JSON publisher. Empty remains truly empty after Delete; 600 seconds is the freshness default | ✅ 🧪 | [`mqtt_ha.cpp`](../main/mqtt_ha.cpp), [`logic/reference_temperature.hpp`](../main/logic/reference_temperature.hpp), [`logic/heating_curve_mqtt.hpp`](../main/logic/heating_curve_mqtt.hpp), [`http_config.cpp`](../main/http_config.cpp), [`www/js/settings.js`](../main/www/js/settings.js) |
...
| 67 | **Deterministic heating-curve diagnosis — raw, heating-only and write-free.** The version-2 allocation- and I/O-free sampler records the canonical room deviation unchanged at most once per 30 minutes, but only after HomeHub register 53 proves normal space operation and input register 38 proves Heating rather than Cooling. Missing/stale room data, an invalid clock, disconnected X10A/HomeHub or unknown mode/gate blocks; idle summer operation and cooling are neutral holds. The timestamped MQTT room mapping alone derives arming; Open-Meteo is optional comparison evidence, so deleting a location stops forecast traffic without stopping local samples. Transient holds/blocks preserve cadence and the durable absolute sample timestamp plus sequence, while deleting the room source disarms and clears sample memory. There is no P gain, deadband, rounding, clamp, slew limit, requested LWT offset, mode switch or active controller. `/status.heating_curve` and the schema-versioned `<base>/heating_curve` payload expose raw current/last error, heating gates, source time, event timestamp/sequence and counters below grouped `room` and `diagnosis` objects. Those domain fields are absent from `<base>/heartbeat`, which is technical board/link health only. The always-rendered card states recording, optional-forecast degradation, summer wait, excluded cooling or the exact missing input, and explains that room kelvin is not calibrated water kelvin. Dynamic LWT actuation remains retired (#294); the read-only Modbus core contains neither write function codes nor value encoders | ✅ 🧪 | [`logic/heating_curve_diagnosis.hpp`](../main/logic/heating_curve_diagnosis.hpp), [`logic/heating_curve_mqtt.hpp`](../main/logic/heating_curve_mqtt.hpp), [`mqtt_ha.cpp`](../main/mqtt_ha.cpp), [`http_status.cpp`](../main/http_status.cpp), [`hp_modbus.cpp`](../main/hp_modbus.cpp), [`www/js/dashboard.js`](../main/www/js/dashboard.js), [`test_heating_curve_diagnosis_contract.mjs`](../test/test_heating_curve_diagnosis_contract.mjs) |
| 68 | **Source-boundary contract gate** — assertions about `main/*.cpp` that the host suite structurally cannot make. `test_logic.cpp` links the IDF-free headers, so it proves what a rule *decides* and never that the firmware still calls it from the right task, in the right order, or from the only file entitled to. Four contracts cover the X10A-gated MQTT lifecycle, the single X10A-free retained-tombstone exception, explicit-action-only mDNS browse and the heating-curve consent/gate boundary. The latter recursively walks all 169 C++ files below `main/`, pins room subscription consent, optional forecast arming, HomeHub→plant→heating-mode→room→X10A order, durable event fields and absence of write entry points, actuator types, FC06/FC16 vocabulary, write-response parsing and value encoders. One script globs `test/test_*_contract.mjs`; matching nothing exits 2, so a vanished suite never looks green | ✅ | [`run-contract-tests.sh`](../scripts/run-contract-tests.sh), [`test_mqtt_x10a_gate_contract.mjs`](../test/test_mqtt_x10a_gate_contract.mjs), [`test_mqtt_source_cleanup_contract.mjs`](../test/test_mqtt_source_cleanup_contract.mjs), [`test_homehub_discovery_contract.mjs`](../test/test_homehub_discovery_contract.mjs), [`test_heating_curve_diagnosis_contract.mjs`](../test/test_heating_curve_diagnosis_contract.mjs) |
...
- **✅ 🧪 HA MQTT auto-discovery.** One retained discovery config per value of the active profile
  ...
  structural `binary_semantic` metadata gives the 3-way valve selector a named state and lets the two
  Smart-Grid contact bits form one four-state UI row. The 2-way-valve output carries the same marker
  but stays **ON/OFF**: it is separate from the configured/current operating mode, so promoting its
  `OFF` to Cooling would state a mode the bit does not establish. Every published boundary remains numeric 0/1.
