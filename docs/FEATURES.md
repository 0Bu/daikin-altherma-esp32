...
- **🧪 What's covered** — CRC & framing (`crc.hpp`), value converters (`convert.hpp`), register
  ...
  *both* sides and cannot unbalance them — while CT does not: the heater's kilowatts land in the
  ...
  the **availability ledger** (`availability.hpp` — the two adjudicated per-row verdicts of #209,
  asserted against all 45 catalog profiles: that each rule selects the quantity it was adjudicated
  for, that no core hydronic row is ever touched, that page `0x10` is still queried after the
  quarantine, and that a legitimate 0 °C reading on any other row still publishes),
