...
// A catalog row's HA entity id. The label alone is NOT enough (legacy-221): it is unique only
// within its register page, while `uniq_id` and the discovery TOPIC are both flat namespaces. The
// catalog carries "Error Code" on the outdoor page AND on the hydronic one, and before this the
// second discovery config landed on the first one's topic under the first one's id — so the broker
// kept one payload, HA created one entity, and a unit reporting two faults showed one. Nothing
// errored; in HA it reads as a sensor the model does not have.
//
...
// The topic shapes a value's discovery config was published on by builds BEFORE legacy-221, when
// the object segment was the bare label slug with no register group. TWO exist per row: every build
// ever wrote the `sensor` form, and builds after the binary_sensor split wrote the `binary_sensor`
// form for a bit-flag row. Both are RETAINED, so both outlive an upgrade as permanently-unavailable
// duplicate entities unless the bridge deletes them (zero-length retained publish) — the
// RETIRED_CRASH_SENSORS pattern, except the old topic is derivable from the row instead of needing
...
inline std::string discovery_config(const std::string& node, const std::string& board_id,
    ...
    std::string j = "{";
    // name/uniq_id are the ENTITY identity and carry the group (legacy-221); `obj` below is the
    // STATE key and must not — it is what mqtt_group.hpp nests and what VictoriaMetrics is keyed on
    // (legacy-217).
...
// legacy-209 defect 4). It lives in the same group object as the row it is derived from, so its
// JSON key needs no prefix — but HA entity ids share one flat namespace across groups, and a
// profile carries an error class on BOTH the outdoor and the hydronic page, so the entity id and
// name are scoped by the group. `<group>_<key>` and "<Group> <Name>": outdoor_state_error_active,
// "Outdoor State Error Active".
//
...
// nothing about the plant, so it is not the kind of per-label domain guess ha_component
// deliberately declines to make for the catalog rows.
