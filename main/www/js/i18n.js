...
const I18N = {
  en: {
    ...
    "hist.circ_on": "Pump running", "hist.circ_off": "Pump off",
    "hist.circ_aria": (l, d) => `${l} — circulation-pump timeline. ${d}. Arrow keys read individual samples.`,
    "hist.valve_heat_total": (d) => `Heating position · ${d}`,
    "hist.valve_cool_total": (d) => `Cooling position · ${d}`,
    "hist.valve_heat": "Heating", "hist.valve_cool": "Cooling",
    "hist.valve_heat_aria": (l, d) => `${l} — 2-way-valve timeline. ${d}. Arrow keys read individual samples.`,
    "hist.flow_switch_total": (d) => `Flow proven · ${d} sampled raster time`,
    "hist.flow_switch_on": "Flow proven", "hist.flow_switch_off": "No flow proof",
    "hist.flow_switch_none": "No closed flow-switch state observed in the recorded period.",
    ...
    "schem.flow_switch": "FLOW SWITCH", "schem.valve_heat": "2WV",
    ...
    "dyn.card_help": "Every 30 minutes, records the raw reference-room deviation during confirmed space-heating operation. Grouped by outdoor temperature and compared with run time, minimum-LWT clipping and thermostat duty, the seasonal trend can indicate whether the heating curve tends to run high or low. It does not convert room kelvin one-to-one into leaving-water kelvin. Nothing is ever written to the heat pump; the firmware has no write path.",
    ...
    "env.pins_hint": "SDA = data (yellow Grove wire); SCL = clock (white Grove wire).",
    ...
    "card.hw_led_detail": (kind, pin, active) => `Status LED: ${kind} on GPIO${pin}${active ? `, ${active}` : ""}.`,
    "card.hw_led_disabled": "Status LED: disabled.",
    "card.hw_btn_detail": (pin, active) => `Reset button: GPIO${pin}, ${active}.`,
    "card.hw_btn_disabled": "Reset button: not configured.",
    "card.hw_env_detail": (sda, scl) => `Outdoor sensor: ENV III with SDA on GPIO${sda} and SCL on GPIO${scl}.`,
    "card.hw_env_disabled": "Outdoor sensor: not configured.",
  ...
  de: {
    ...
    "hist.circ_on": "Pumpe läuft", "hist.circ_off": "Pumpe aus",
    "hist.circ_aria": (l, d) => `${l} — Verlauf der Zirkulationspumpe. ${d}. Mit den Pfeiltasten einzelne Zeitpunkte ablesen.`,
    "hist.valve_heat_total": (d) => `Heizstellung · ${d}`,
    "hist.valve_cool_total": (d) => `Kühlstellung · ${d}`,
    "hist.valve_heat": "Heizen", "hist.valve_cool": "Kühlen",
    "hist.valve_heat_aria": (l, d) => `${l} — Verlauf des 2-Wege-Ventils. ${d}. Mit den Pfeiltasten einzelne Zeitpunkte ablesen.`,
    "hist.flow_switch_total": (d) => `Strömung bestätigt · ${d} Rasterzeit`,
    "hist.flow_switch_on": "Strömung bestätigt", "hist.flow_switch_off": "Keine Strömungsfreigabe",
    "hist.flow_switch_none": "Kein geschlossener Strömungsschalter im aufgezeichneten Zeitraum erfasst.",
    ...
    "schem.r2t": "R2T", "schem.r3t": "R3T",
    "schem.flow_switch": "STRÖMUNG", "schem.valve_heat": "2WV",
    ...
    "dyn.card_help": "Zeichnet alle 30 Minuten die rohe Abweichung des Referenzraums während eindeutig bestätigtem Raumheizbetrieb auf. Nach Außentemperatur gruppiert und zusammen mit Laufzeit, Mindest-Vorlauf-Clipping und Thermostat-Einschaltdauer kann der Saisontrend zeigen, ob die Heizkurve tendenziell zu hoch oder zu niedrig liegt. Raum-Kelvin werden nicht 1:1 in Vorlauf-Kelvin umgerechnet. An die Wärmepumpe wird nichts geschrieben; die Firmware hat keinen Schreibpfad.",
    ...
    "env.pins_hint": "SDA = Datenleitung (gelbe Grove-Leitung), SCL = Taktleitung (weiße Grove-Leitung).",
    ...
    "circ.running": "Läuft", "circ.stopped": "Aus", "circ.checking": "Prüft",
    ...
    "card.hw_led_detail": (kind, pin, active) => `Status-LED: ${kind} auf GPIO${pin}${active ? `, ${active}` : ""}.`,
    "card.hw_led_disabled": "Status-LED: deaktiviert.",
    "card.hw_btn_detail": (pin, active) => `Reset-Taster: GPIO${pin}, ${active}.`,
    "card.hw_btn_disabled": "Reset-Taster: nicht konfiguriert.",
    "card.hw_env_detail": (sda, scl) => `Außensensor: ENV III mit SDA auf GPIO${sda} und SCL auf GPIO${scl}.`,
    "card.hw_env_disabled": "Außensensor: nicht konfiguriert.",
