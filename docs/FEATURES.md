...
| 20 | **IDF-free host-tested logic core** (2682 checks, re-derived — see §8) | 🧪 | [`main/logic/`](../main/logic), [`test/test_logic.cpp`](../test/test_logic.cpp) |
...
| 67 | **Deterministic heating-curve diagnosis — a write-free P controller, armed by its own sources.** A pure, allocation- and I/O-free controller consumes the canonical living-room error plus X10A, HomeHub and plant-gate evidence. Gain 1, ±0.25 K deadband, whole-kelvin quantization, ±2 K envelope, 1 K/decision slew limit and a 30-minute cadence are fixed and host-tested. Missing/stale room data, an invalid clock, disconnected X10A/HomeHub or unknown plant gate fails closed; an inactive plant holds. Missing forecast is an explicit degraded state with a zero contribution. ARMING IS DERIVED, not switched or stored: `dynamic_lwt_armed()` answers it from the two sources the diagnosis reads — the MQTT room mapping and a forecast location — so deleting either disarms it on the next cycle and nothing persisted can disagree with the configuration a reader sees. There is no mode enum, no Config field and no route; blob v14's byte is written zero and ignored, so `active` is unrepresentable everywhere. The switch this replaced could not be reached: it answered 409 until those sources existed, while their only editors lived inside the card it revealed. THE GATE ORDER is part of the contract — the plant gate is asked BEFORE the room source, because both fail together outside the heating season (a summer-off room thermostat is not control-eligible) and answering "room unavailable" made a plant resting exactly as it should read as a standing fault: the reference installation stood at failsafes=2734, holds=0 in August with nothing wrong. The always-rendered bottom-most **Heating-curve diagnosis** card — always, because it is where both sources are configured — carries four rows — live status, room source, forecast, method — and deliberately not a fifth: it neither restates the Firmware toggle as a second "operating mode" readout nor carries a constant "safety: read-only" line, because with the write path deleted neither could ever say anything else. Its status row names why no proposal exists yet and keeps an idle plant NEUTRAL rather than a warning, since that is the expected reading for the whole non-heating half of the year; an unconfigured source is dim for the same reason. Where the block IS the room source it prints that source's own reason ("room thermostat switched off", "room reading too old") from one shared table the source's own tongue reads too, and the source row turns amber to match — it previously reported "room input unavailable" while that source rendered green and current two rows below. The expandable tongues explain status, quantities, freshness and provenance without the former duplicate green summary lines. Protocol and Firmware rows use the same label-owned explanation pattern while keeping values and controls independent. `/status.dynamic_lwt.armed` and flat numeric `lwt_controller_*` heartbeat fields (`lwt_controller_armed` replacing `lwt_controller_mode`) expose terms, bounds, reasons, source age, gates and counters for VictoriaMetrics. The controller has no actuator dependency and none exists to depend on: dynamic LWT actuation is retired (#294) and a source-boundary CI contract walks every firmware source to prove no write capability remains | ✅ 🧪 | [`logic/dynamic_lwt_controller.hpp`](../main/logic/dynamic_lwt_controller.hpp), [`logic/heartbeat.hpp`](../main/logic/heartbeat.hpp), [`mqtt_ha.cpp`](../main/mqtt_ha.cpp), [`http_status.cpp`](../main/http_status.cpp), [`http_config.cpp`](../main/http_config.cpp), [`www/js/dashboard.js`](../main/www/js/dashboard.js), [`test_dynamic_lwt_shadow_contract.mjs`](../test/test_dynamic_lwt_shadow_contract.mjs) |
| 68 | **Source-boundary contract gate** — assertions about `main/*.cpp` that the host suite structurally cannot make. `test_logic.cpp` links the IDF-free headers, so it proves what a rule *decides* and never that the firmware still calls it from the right task, in the right order, or from the only file entitled to: "no firmware source can issue a Modbus write" is a claim about a whole component, and the only instrument that can settle it is the source text. Four contracts today — the X10A-gated MQTT lifecycle, the single X10A-free retained-tombstone exception, the explicit-action-only mDNS browse, and the dynamic-LWT consent boundary (that a saved source is what starts its subscription or its fetch, that arming is derived rather than stored, that the plant gate is evaluated before the room source, and — since the write path was retired — that no write entry point, actuator type, FC06/FC16 builder or issued write function code exists in any of the 119 scanned sources). They ran as bare `node test/…` CI steps with **no local entry point**, so the local loop silently skipped them and a new sibling would have been invisible to every entry point at once; one script now globs `test/test_*_contract.mjs`, so a new contract joins the gate with no workflow edit — the property `test_ui_*.mjs` already had. A glob matching nothing exits 2 rather than reporting success, since "no tests found" must never look like "all tests passed" in the script written to stop a check from disappearing | ✅ | [`run-contract-tests.sh`](../scripts/run-contract-tests.sh), [`test_mqtt_x10a_gate_contract.mjs`](../test/test_mqtt_x10a_gate_contract.mjs), [`test_mqtt_source_cleanup_contract.mjs`](../test/test_mqtt_source_cleanup_contract.mjs), [`test_homehub_discovery_contract.mjs`](../test/test_homehub_discovery_contract.mjs), [`test_dynamic_lwt_shadow_contract.mjs`](../test/test_dynamic_lwt_shadow_contract.mjs) |
...
- **✅ 🧪 Second SOURCE: Modbus TCP to a Daikin HomeHub (EKRHH)**
  ...
  **no cost when absent** — an empty address creates no task, socket, discovery or requests. This is
  ...
  restore/conflict state machine — was **removed** together with the FC06/FC16 request builders, so
  no source file can frame or issue a Modbus write. No new ESP-IDF component: `mdns` and `lwip` were
  already linked.
