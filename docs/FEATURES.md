...
| 11 | Core-dump-to-flash + summary capture + offline symbolication, with a proven **orphan dump** (foreign `app_elf_sha256`) erased at boot or suppressed if that erase fails, so `coredump` never offers an undecodable download | ✅ 🧪 | [`diag_crash.cpp`](../main/diag_crash.cpp), [`logic/crashinfo.hpp`](../main/logic/crashinfo.hpp), [`decode-coredump.sh`](../scripts/decode-coredump.sh) |
...
| 20 | **IDF-free host-tested logic core** (2684 checks, re-derived — see §8) | 🧪 | [`main/logic/`](../main/logic), [`test/test_logic.cpp`](../test/test_logic.cpp) |
...
| 33 | **Detect-sweep heap and log hardening** — install-once UART + register-only pin remap (no per-swap driver realloc) and silent-bus detect backoff, closing a fragmentation panic caught by a symbolized coredump. The fingerprint sweep probes the union of every model's pages, so expected missing/rejected pages stay quiet while corrupt frames and the final no-link diagnosis remain visible | ✅ 🧪 | [`hp_comm.cpp`](../main/hp_comm.cpp), [`logic/uart_plan.hpp`](../main/logic/uart_plan.hpp), [`logic/hp_query_log.hpp`](../main/logic/hp_query_log.hpp), [`hp_poll.cpp`](../main/hp_poll.cpp), [`logic/detect_backoff.hpp`](../main/logic/detect_backoff.hpp) |
...
| 38 | **Explicit board identity + runtime hardware config** (`POST /set_board`) — the stable preset id (`m5stack_atoms3_lite`, `seeed_xiao_esp32s3`, or `custom`) is CRC-protected and atomically persisted with indicator pin/driver/polarity + button pin in blob v12. `/status.board.preset_id`/`.preset_name` report the actual selection; board name and ENV-III vendor support are never inferred from matching GPIOs. Unknown ids are rejected, while LED/reset customization retains the explicitly selected physical board; only choosing Custom removes that identity. Pre-v12 `board_set` values migrate only after an explicit historical save. Per-board default values remain host-tested against the request validator, and the indicator announces its resolved pin/driver on `/diag` | ✅ 🧪 | [`http_config.cpp`](../main/http_config.cpp), [`logic/config_store.hpp`](../main/logic/config_store.hpp), [`logic/config_model.hpp`](../main/logic/config_model.hpp), [`logic/board_presets.hpp`](../main/logic/board_presets.hpp), [`http_status.cpp`](../main/http_status.cpp), [`status_led.cpp`](../main/status_led.cpp) |
| 39 | **Stack-overflow watchpoint** — a hardware watchpoint on every task's stack limit, so the *first* write past it panics at the offending instruction instead of corrupting a neighbour silently. IDF's default canary is only compared at a context switch and a sparsely-writing frame can step over it — which is how a v1.0.12 `httpd` overflow overwrote its own TCB and died 44 s later in unrelated lwip code. The `/status` and `/values` JSON use `+=` rather than long `+` chains and now run only on `httpd`; `hp_poll` remains at its independently sized 8 KB, while `httpd` is 16 KB after the signed dev.295/dev.296 ELFs measured the ENV-III-expanded status frame growing from 9776 to 10576 bytes and the dev.296 live OTA reproduced TCB corruption in its nested `config()` copy with only 1712 bytes left | ✅ | [`sdkconfig.defaults`](../sdkconfig.defaults), [`http_server.cpp`](../main/http_server.cpp), [`http_status.cpp`](../main/http_status.cpp) |
...
| 42 | **24-hour trend rings** — a fixed-cadence (5 min × 288) `int16` ring per trended value or state, **static** rather than on the heap (576 B each; **25 X10A/board/state rings = 14400 B** plus **ten label-free HomeHub rings = 5760 B**, **20160 B total**, guarded by separate ceiling asserts). Their non-zero `pending` sentinel places the rings in `.data`, not `.bss`, so each also costs its size in the flash image. Chunked `GET /history` serves X10A by default and `source=modbus` for the second instrument; the UI draws the result in the value-row explainer and schematic inspector. Six numeric pairings overlay blue X10A and petrol HomeHub lines on one aligned scale. Six categorical timelines cover Smart-Grid mode, the tank heater, BUH, the 3-way valve, Quiet and Defrost: Boost marks mode 2; BSH pairs converter-qualified X10A BSH with HomeHub input 32 and latches any observed ON for the open bucket; BUH event-folds its converter-qualified Step1/Step2 bits independently and combines them into Aus/Stufe 1/Stufe 2; the valve pairs converter 306 with HomeHub input 37 and retains the latest sampled position; Quiet pairs converter 301 with HomeHub input 9 and retains the latest sampled mode; Defrost is X10A-only and event-folded. The UI calls these sampled raster intervals, not exact runtime. There is no heater-specific power register: HomeHub input 51 remains whole-system electrical input. Two X10A absences remain distinguished: a register that did not answer vs. an outdoor-unit page held over while the compressor rests. Five displayed figures (pump speed, ΔT, heat output, electrical input, COP) plus the combined BUH state have no single ring matching the shown figure; `www/js/history.js` assembles their curves/timeline from source rings using the live expressions. Two primary trends are the board's own **free heap** and **largest contiguous block**. Numeric row trends are addressed structurally by **(register page, byte offset, unit)**, shared-bit trends add the converter, and Smart-Grid combines two structurally identified contacts. HomeHub history membership is explicit in `HOMEHUB_HISTORIES`; compile-time guards pin it to real trends and unique offsets | ✅ 🧪 | [`logic/history.hpp`](../main/logic/history.hpp), [`logic/homehub_map.hpp`](../main/logic/homehub_map.hpp), [`history.cpp`](../main/history.cpp), [`http_status.cpp`](../main/http_status.cpp), [`www/js/history.js`](../main/www/js/history.js) |
...
| 50 | **Per-row availability ledger** — is a decoded number a *measurement*, or merely something the firmware could decode? The wire format's own `0x8000` marker, the plausibility envelope (#35) and the generator's detect-only flag each answer a narrower question; the ledger records the residue with evidence beside each structural `(page, offset, converter)` key. Three verdicts are live. `ZeroMeansAbsent` withholds `Target Cond. Temp.`'s exact raw `0x0000`, flat through a full compressor cycle while the inverter reached 32 rps and the discharge pipe passed 100 °C. `ZeroPageMeansAbsent` resolves the safe page-level subset of [#224](https://github.com/0Bu/daikin-altherma-esp32/issues/224): page `0xA1` (second-outdoor-unit Water-HX) returned 16 zero bytes including its unit-family flags, so its four analog rows are absent together; an individual 0 °C remains publishable whenever any byte proves the page populated, and a reply too short to reach the flags proves nothing. `AboveRangeIsAbsent` refuses the expansion-valve pulse rows' discrete `0xFFF8` outlier after 30 days otherwise spanning 0–474 pulses; both unsigned 65528 and signed −8 are impossible positions, so the documented `u16` decode stays untouched. `Unproven` still exists with no live entry: it held `Target Evap. Temp.` until #194 proved a wrong converter rather than an absent measurement. A catalog test pins every rule across all 45 profiles, including exactly 80 `0xA1` analog rows. Deliberately contextual: a global "0 °C is unavailable" rule would destroy real thermistor and idle-current readings | ✅ 🧪 | [`logic/availability.hpp`](../main/logic/availability.hpp), [`hp_convert.cpp`](../main/hp_convert.cpp), [`hp_poll.cpp`](../main/hp_poll.cpp), [`mqtt_ha.cpp`](../main/mqtt_ha.cpp) |
...
| 61 | **Second SOURCE: Modbus TCP to a Daikin HomeHub (EKRHH)** — an optional stack beside X10A, not an alternative: separate task/cache/link state, explicit mDNS search, request-bound reads, independently attributed UI/history/MQTT values (including the paired BSH, 3-way-valve and Quiet timelines) and zero steady-state cost without a configured address. X10A remains primary where both describe one quantity. The link is READ-ONLY — the register-54 actuator built for #300 was removed with the retirement of dynamic LWT actuation (#294), so no source file can frame or issue a Modbus write — and Home Assistant, MQTT, HTTP and MCP expose no command or raw-register surface | ✅ 🧪 | [`hp_modbus.cpp`](../main/hp_modbus.cpp), [`logic/modbus.hpp`](../main/logic/modbus.hpp), [`logic/homehub_map.hpp`](../main/logic/homehub_map.hpp), [`MODBUS_PROTOCOL.md`](MODBUS_PROTOCOL.md), [`MODBUS_ACTUATION.md`](MODBUS_ACTUATION.md) |
| 62 | **Configurable MQTT living-room source — canonical dynamic-LWT input.** Settings stores one presentation name, one exact MQTT topic (wildcards refused), required dot-separated paths for current temperature, target temperature and source timestamp, optional `enabled` and HVAC-mode paths, and a 10–3600 s maximum age. A non-empty mapping is test-before-persist: Save temporarily subscribes on the existing authenticated esp-mqtt client without changing Config/NVS, decodes the same bounded JSON path used at runtime, and issues a mapping-bound proof only after current/target values are finite, inside the fixed 5–35 °C plausibility envelope, and source time is fresh. The saved source is subscribed and decoded only while the separate default-OFF dynamic-LWT Firmware switch selects SHADOW; OFF removes the live subscription and captured runtime sample without deleting the mapping, while the independent Test path remains available for pre-enable proof. Optional gates are type-checked when configured; disabled or non-heating state may be saved but remains explicitly ineligible. Retained data without source time, stale/future/backward timestamps, missing fields and out-of-range values fail closed. Blob v13 appends the three readiness mappings without changing older layouts; migrated sources remain configurable but cannot become control-eligible until a target path is deliberately saved. `/status.reference_temperature` exposes the raw decoded values and the canonical accepted verdict with stable source id `living_room`, fixed calibration `0`, validity flags, `room_error_k`, stable reason/code and counters. The flat numeric heartbeat archives the accepted view for VictoriaMetrics; `room_error_k` exists only when all configured gates pass. `active` is deliberately not a validity gate. No MQTT command subscriber or direct heat-pump write is added here; the HomeHub link is read-only. The UI ships no vendor preset: a Meross MTS200 mapping is ordinary configuration, for example `thermostat.current_temperature_c`, `thermostat.target_temperature_c`, `thermostat.read_at`, optionally `thermostat.enabled` and `thermostat.hvac_mode`. When no source is configured — including immediately after Delete — every mapping field opens empty; 600 seconds remains the default freshness limit | ✅ 🧪 | [`mqtt_ha.cpp`](../main/mqtt_ha.cpp), [`logic/reference_temperature.hpp`](../main/logic/reference_temperature.hpp), [`logic/heartbeat.hpp`](../main/logic/heartbeat.hpp), [`http_config.cpp`](../main/http_config.cpp), [`www/js/settings.js`](../main/www/js/settings.js) |
| 63 | **HomeHub write path — RETIRED** — #300/PR #309 built a single-writer register-54 actuator (coalescing mailbox, typed LWT-offset intents, signed `Int16` −10..+10 K descriptor, fresh FC03 baseline → FC06 bound echo → independent FC03 readback, conflict-stops-instead-of-reasserting, restore on disable/stop, default-off with unresolved boot ownership). It was never commissioned — zero requests, zero write attempts — and was **deleted** when #294 retired dynamic LWT actuation, along with `actuation_enabled`, the `/status.modbus.actuator` object, the `modbus_actuator_*` heartbeat fields, the evcc intent envelope and the FC06/FC16 request builders. "Nothing is written" is now a property of the code, not of a test asserting nobody calls it; the contract test walks every firmware source to keep it so. The **plant gate** (input register 53) is the one piece kept, as an ordinary read | ⛔ retired | [`MODBUS_ACTUATION.md`](MODBUS_ACTUATION.md), [`test_dynamic_lwt_shadow_contract.mjs`](../test/test_dynamic_lwt_shadow_contract.mjs) |
| 64 | **Direct Open-Meteo forecast on the ESP32.** Configured latitude/longitude selects one saved location; MQTT, evcc and k3s are not dependencies. Only while the separate default-OFF dynamic-LWT Firmware switch selects SHADOW does the dedicated task send coordinates and the device's public source IP: it waits for WiFi and synchronized time, then fetches a bounded six-hour JSON response over CA-verified HTTPS every 45 minutes with a 5-minute error retry. OFF wakes and pauses the task, clears its runtime forecast/timestamps while preserving the saved location, and retracts the retained MQTT evidence document. The query pins `icon_seamless`, asks only for `temperature_2m` and `shortwave_radiation`, uses Unix/GMT timestamps and validates both units before deriving the next two complete hours' mean °C and Wh/m². Fetch/decision times, explicit unavailable provider issue time, location, model, availability, freshness and counters are exposed in `/status`; coordinates are redacted in diagnostic snapshots. If MQTT, Open-Meteo and SHADOW are enabled, `<base>/weather/openmeteo/forecast` retains one atomic evidence snapshot containing the derived values, provider/model, fetch/forecast/valid-until times and fail-closed availability/freshness flags; it deliberately contains no coordinates and creates no HA Discovery entities. Clearing both location fields queues one QoS-1 retained empty tombstone after the disabled state is persisted, so the broker definitively removes the topic even without X10A. Upgrade cleanup retracts the four weather configs published by the earlier short-lived contract and removes its retained `<base>/weather_forecast` predecessor when present. Failed refreshes keep the old figures visible only for diagnosis while `available`/`fresh` fail closed. Blob v10 persists signed microdegrees plus an explicit enable bit, and `POST /set_weather` only saves/wakes so network work never blocks httpd. A latitude/longitude pair copied from Google Maps can be pasted into either coordinate field; the UI splits, range-checks and rounds it to the six decimal places persisted by the firmware. Nothing is persisted before Save and nothing is sent while the feature switch is OFF. The modal discloses location/IP transfer and attributes Open-Meteo/DWD ICON. The public endpoint is intended for non-commercial use and has no uptime guarantee; the provider response is not stored or backfilled and no forecast is used for heat-pump writes yet | ✅ 🧪 | [`weather_forecast.cpp`](../main/weather_forecast.cpp), [`logic/open_meteo.hpp`](../main/logic/open_meteo.hpp), [`logic/weather_forecast.hpp`](../main/logic/weather_forecast.hpp), [`logic/weather_mqtt.hpp`](../main/logic/weather_mqtt.hpp), [`http_config.cpp`](../main/http_config.cpp), [`http_status.cpp`](../main/http_status.cpp), [`www/js/settings.js`](../main/www/js/settings.js) |
| 65 | **Optional M5Stack ENV III outdoor-climate input** — one runtime-configured I²C bus reads SHT30 temperature/humidity (`0x44`, CRC-checked) and QMP6988 pressure (`0x70`, calibrated compensation) as one atomic sample every 10 s. This is specifically an ENV III driver; ENV/II/IV use different sensor IC combinations and are not claimed as compatible. Its configuration is a conditional section inside **Board Hardware**, visible and enabled only for a pending board identity tagged with vendor `m5stack`; it is no longer a separate Dynamic-control row or popup. Seeed and Custom fail closed, while another future M5Stack preset automatically inherits the capability. It remains opt-in, rejects GPIO collisions with X10A/LED/button on save and boot, exposes explicit support/freshness/error/counters in `/status.env3`, and renders an independent dashboard card rather than pretending to replace Daikin R1T. The section has one sensor dropdown (`No sensor` / currently only `ENV III`) instead of a separate enable checkbox. `No sensor` is the default and hides/disables the SDA/SCL selectors; saving it submits an explicit disable, releases both GPIO reservations and retracts MQTT/HA state without requiring hardware. Selecting `ENV III` reveals the two selectors plus the data/clock, Grove-wire and GPIO wiring explanation. GPIO2/1 is preselected when Grove is free; otherwise only safe free pins are shown. I²C can identify devices on an already configured bus but cannot discover their physical GPIO pair. Board identity, LED/button and ENV III are submitted to `POST /set_board` as one proposed snapshot. Enable is test-before-persist: Save first requires a CRC-valid SHT30 sample and the QMP6988 `0x5c` chip id, leaves Board Hardware open with a localized component error on failure, and only then performs one atomic NVS write/reboot. The standalone `/set_env3` route retains the same contract for compatibility. An unchanged active mapping requires a fresh runtime sample; moving an owned bus is disable-first. Each fresh sample is retained on `<base>/env3` as flat numeric JSON. Three HA-discovery measurement sensors expose temperature, humidity and air pressure; each requires both the device LWT and its own JSON key, so errors/staleness (`{}`) mark only those entities unavailable. Disabling ENV III retracts the state topic and all three retained discovery configs. No control consumer is added | ✅ 🧪 | [`env3.cpp`](../main/env3.cpp), [`logic/env3.hpp`](../main/logic/env3.hpp), [`logic/board_presets.hpp`](../main/logic/board_presets.hpp), [`logic/discovery.hpp`](../main/logic/discovery.hpp), [`mqtt_ha.cpp`](../main/mqtt_ha.cpp), [`http_status.cpp`](../main/http_status.cpp), [`www/js/dashboard.js`](../main/www/js/dashboard.js), [`BOARDS.md`](BOARDS.md) |
| 66 | **Complete UI interaction merge gate** — the assembled production UI is executed in a deterministic DOM harness, not merely parsed. Its explicit matrix must cover every modal id in the production `MODALS` registry and drives Settings/Back, every visible modal entry action, open, Cancel, backdrop, Escape, accepted/rejected Save, invalid input, integrated board-dependent ENV III states and the two-step bug-report path. Settings and every modal also have unique stable hash routes; the matrix proves reload-style direct restoration, browser Back/Forward, normal status-backed form filling, draft-safe first-status hydration, and URL return on every close path. Modal lifecycle assertions also require both document scrollers to lock while a dialog is open and release on every close path. A focused mobile-layout contract pins `100dvh`, the bounded card scroll container, iOS momentum scrolling and overscroll containment; the selftest removes the dynamic viewport again and must catch the original iPhone failure. A central command also discovers every `test_ui_*.mjs` contract, HomeHub discovery and the MCP page; its other selftest mutates Board Hardware's integrated ENV III close action to an undefined helper and requires that defect to be caught, then simulates CLI and app-MCP merges to prove a failed suite or stale review stamp returns the blocking exit code. The same command runs in CI's required `gates` job. For UI-relevant diffs, the `/ui-use-case-review` skill adds narrow/desktop click-through, and a conditional PreToolUse hook requires a checked head-SHA-stamped review record and reruns the deterministic suite immediately before command-line merge | ✅ 🧪 | [`test_ui_use_cases.mjs`](../test/test_ui_use_cases.mjs), [`test_ui_modal_scroll.mjs`](../test/test_ui_modal_scroll.mjs), [`run-ui-use-case-tests.sh`](../scripts/run-ui-use-case-tests.sh), [`selftest.sh`](../tools/ui/selftest.sh), [`SKILL.md`](../.claude/skills/ui-use-case-review/SKILL.md), [`require-ui-use-case-review.sh`](../.claude/hooks/require-ui-use-case-review.sh) |
| 67 | **Deterministic dynamic-LWT P controller in SHADOW only.** A pure, allocation- and I/O-free controller consumes the canonical living-room error plus X10A, HomeHub and plant-gate evidence. Gain 1, ±0.25 K deadband, whole-kelvin quantization, ±2 K envelope, 1 K/decision slew limit and a 30-minute cadence are fixed and host-tested. Missing/stale room data, an invalid clock, disconnected X10A/HomeHub or unknown plant gate fails closed; an inactive plant holds. Missing forecast is an explicit degraded state with a zero contribution. Blob v14 adds only OFF/SHADOW and every install, migration and reboot defaults to OFF; `active` is not accepted by config or HTTP. A Firmware-card switch is the explicit consent gate: SHADOW requires MQTT, a complete room mapping and HomeHub, starts the saved MQTT/Open-Meteo inputs live, and reveals the bottom-most **Heating-curve diagnosis** card; OFF hides that card, unsubscribes/pauses/clears runtime input evidence and preserves source configuration. Removing any dependency also disarms a stored SHADOW mode. That card carries four rows — live status, room source, forecast, method — and deliberately not a fifth: it neither restates the Firmware toggle as a second "operating mode" readout nor carries a constant "safety: read-only" line, because with the write path deleted neither could ever say anything else. Its status row names why no proposal exists yet and keeps an idle plant NEUTRAL rather than a warning, since that is the expected reading for the whole non-heating half of the year. The expandable tongues explain status, quantities, freshness and provenance without the former duplicate green summary lines. Protocol and Firmware rows use the same label-owned explanation pattern while keeping values and controls independent. `/status.dynamic_lwt` and flat numeric `lwt_controller_*` heartbeat fields expose terms, bounds, reasons, source age, gates and counters for VictoriaMetrics. The controller has no actuator dependency and none exists to depend on: dynamic LWT actuation is retired (#294) and a source-boundary CI contract walks every firmware source to prove no write capability remains | ✅ 🧪 | [`logic/dynamic_lwt_controller.hpp`](../main/logic/dynamic_lwt_controller.hpp), [`logic/heartbeat.hpp`](../main/logic/heartbeat.hpp), [`mqtt_ha.cpp`](../main/mqtt_ha.cpp), [`http_status.cpp`](../main/http_status.cpp), [`http_config.cpp`](../main/http_config.cpp), [`www/js/dashboard.js`](../main/www/js/dashboard.js), [`test_dynamic_lwt_shadow_contract.mjs`](../test/test_dynamic_lwt_shadow_contract.mjs) |
| 68 | **Source-boundary contract gate** — assertions about `main/*.cpp` that the host suite structurally cannot make. `test_logic.cpp` links the IDF-free headers, so it proves what a rule *decides* and never that the firmware still calls it from the right task, in the right order, or from the only file entitled to: "no firmware source can issue a Modbus write" is a claim about a whole component, and the only instrument that can settle it is the source text. Four contracts today — the X10A-gated MQTT lifecycle, the single X10A-free retained-tombstone exception, the explicit-action-only mDNS browse, and the dynamic-LWT consent boundary (that OFF costs no subscription and no outbound request, and — since the write path was retired — that no write entry point, actuator type, FC06/FC16 builder or issued write function code exists in any of the 119 scanned sources). They ran as bare `node test/…` CI steps with **no local entry point**, so the local loop silently skipped them and a new sibling would have been invisible to every entry point at once; one script now globs `test/test_*_contract.mjs`, so a new contract joins the gate with no workflow edit — the property `test_ui_*.mjs` already had. A glob matching nothing exits 2 rather than reporting success, since "no tests found" must never look like "all tests passed" in the script written to stop a check from disappearing | ✅ | [`run-contract-tests.sh`](../scripts/run-contract-tests.sh), [`test_mqtt_x10a_gate_contract.mjs`](../test/test_mqtt_x10a_gate_contract.mjs), [`test_mqtt_source_cleanup_contract.mjs`](../test/test_mqtt_source_cleanup_contract.mjs), [`test_homehub_discovery_contract.mjs`](../test/test_homehub_discovery_contract.mjs), [`test_dynamic_lwt_shadow_contract.mjs`](../test/test_dynamic_lwt_shadow_contract.mjs) |
...
- **✅ 🧪 HA MQTT auto-discovery.** One retained discovery config per value of the active profile
  ...
  un-grouped ids).
  An enabled HomeHub publishes its live register map as flat JSON on `<base>/modbus`, deliberately
  without per-value HA discovery. Connect-time and five-minute passes tombstone the 27 `_modbus`
  discovery topics emitted by builds through `v1.0.0-dev.257`, so existing Modbus entities disappear
  while the data stream remains. A dead link yields `{}` and a disabled stack retracts the value
  topic. The former retained `<base>/modbus/status` is deleted when a bounded probe finds it because
  heartbeat already carries link state, RX and failures.
  Its Int16 enums remain numeric constants on MQTT (mode 2 stays `2`, never `"Recommended on"`); the
  browser names them from separate semantic metadata. Bounded read-only migration probes delete
  legacy retained `<base>/state` and `<base>/modbus/status` values when they exist; clean brokers
  receive no empty publish on either retired topic during reconnect.
...
- **✅ Save-time broker pre-flight** ([`http_config.cpp`](../main/http_config.cpp)): `POST /set_mqtt`
  ...
  guarded by a largest-free-block check: under heap pressure it rejects the save with a retryable 503
  rather than skip connect/auth or risk OOM-ing the live bridge. Thus every accepted non-empty broker
  has completed MQTT CONNECT; DNS plus an open TCP port alone never authorize persistence. The parsed
  host/port come from the host-tested [`mqtt_uri.hpp`](../main/logic/mqtt_uri.hpp), whose scheme defaults track **esp-mqtt's
...
- **Read-only by design.** No command subscriptions and no actuation. The optional inbound reference
  sensor subscription only captures and qualifies an input for display/status; it cannot reach either pump link.
...
- **✅ Core dump to flash (ELF format).** `CONFIG_ESP_COREDUMP_ENABLE_TO_FLASH` into a dedicated
  ...
  capture (`coredump_is_foreign()`, host-tested). Its proven foreign identity stays suppressed for
  the boot if that best-effort erase fails, and `GET /coredump` uses the same policy, so `coredump`
  never advertises a download
...
- **✅ 🧪 Deleting a crash report (`POST /crash/dismiss`)** ([`diag_crash.cpp`](../main/diag_crash.cpp),
  ...
  until the next reload. **Erase first, mark second** for current-firmware evidence — a failed erase
  answers `500` and marks nothing, since a dismissal outliving it would report "no crash" with the
  dump still downloadable. Proven foreign residue is already suppressed from status and download,
  so its failed erase cannot pin a separate current-fault banner. RAM-only by design: after any reboot
  the reason is no longer a fault and the dump is gone, while a
...
- **✅ 🧪 Getting the evidence off the board — and what must not come with it.** Everything above is
  ...
  behind `?redact=1` replaces the reporter-identifying values (`wifi.ssid`/`ip`/`bssid`/`mac`,
  `mqtt.broker`, `syslog.host`, `ntp.server`, `modbus.host`, weather latitude/longitude) and scrubs the log lines that interpolate a host, an IP
...
- **✅ 🧪 Poll engine** ([`hp_poll.cpp`](../main/hp_poll.cpp)): profile registers → query → decode →
  ...
  on for months.
- **✅ 🧪 Second SOURCE: Modbus TCP to a Daikin HomeHub (EKRHH)**
  ...
  **no cost when absent** — an empty address creates no task, socket, discovery or requests. This is
  what let the X10A poll task give its 4 KB back after an
  earlier revision had polled both links from one task.
  ...
  so the next cycle reconnects. **mDNS discovery is explicit**: the dialog's Search button browses
  `_http._tcp` (what the hub advertises), accepts up to 64 responders and filters the `homehub-*`
  hostname — mandatory because this firmware answers that same browse. Three attempts bound that
  one request; a hit fills the editable field but only Save persists it. A manual address uses the
  same field, and empty disables all future activity. A failed connect **backs off** through the X10A
  sweep's own policy without falling back to discovery.
  ...
  There is no actuator below this source path. What #300 built — register 54 through the same
  task-owned socket, with a fixed mailbox, fresh pre-read, bound FC06 echo, FC03 readback and a
  restore/conflict state machine — was **removed** when #294 retired dynamic LWT actuation, together
  with the FC06/FC16 request builders, so no source file can frame or issue a Modbus write; see
  [`MODBUS_ACTUATION.md`](MODBUS_ACTUATION.md). No new ESP-IDF component: `mdns` and `lwip` were
  already linked.
...
- **🧪 What's covered** — CRC & framing (`crc.hpp`), value converters (`convert.hpp`), register
  ...
  link's own `rx`/`tx` (and by ENV III while enabled), because the reservation runs in **every** direction: `board_hw_valid()`
  refuses a local pin that equals either link pin, so a picker still listing them offers a choice
  whose only outcome is a `400`. `ReservedPins` is deliberately anonymous about which set it carries
  (`pin_a`…`pin_d`) — naming its fields after one direction made the others read as a lie — and the
  named factories in `config_model.hpp` state the direction at the call site),
  ...
  pick `POST /set_board` would refuse — it asserts nothing automatically about which board this is),
  plus a stable user-selected id (`Config::board_preset_id`) stored atomically with the hardware in
  blob v12 and served as `/status.board.preset_id`/`.preset_name`. Untouched Kconfig defaults carry no
  identity even though they equal the XIAO fields. A known id must match all preset hardware fields;
  editing them makes the pending selection Custom. ENV III therefore gates on a validated explicit
  M5Stack selection rather than GPIO resemblance. Hardware changes decide the **reboot** (both parts
  are claimed once at task start); identity-only changes require a **save** without a restart
  ...
  **🔭 Modbus TCP framing + HomeHub register codecs** (`modbus.hpp` — MBAP
  ...
  the exact `homehub-*` mDNS filter; the host-tested core used by the independent, read-only
  HomeHub task in `hp_modbus.cpp`), and the SNTP wall-clock RFC 3339
  ...
  *"Fan 1 (10 rpm)"*, so the day the generator emits *"Fan 1 (step)"* the count fails rather than
  ...
  excluding decreases from no-event evidence; the conservative at-or-below-1-bar project pressure
...
| `esp_driver_i2c` | optional ENV III SHT30 + QMP6988 outdoor-climate sensor |
