...
| 20 | **IDF-free host-tested logic core** (1791 checks, re-derived — see §8) | 🧪 | [`main/logic/`](../main/logic), [`test/test_logic.cpp`](../test/test_logic.cpp) |
...
- **🧪 What's covered** — CRC & framing (`crc.hpp`), value converters (`convert.hpp`), register
  ...
  *both* sides and cannot unbalance them — while CT does not: the heater's kilowatts land in the
  ...
  the **24-hour plant checkup** (`checkup.hpp` — the compressor-start edge, where "unknown" must
  never read as "stopped" and a poll gap must never read as a transition; every threshold at its
  boundary; the aggregation property that a half-observed window can never report green; and a
