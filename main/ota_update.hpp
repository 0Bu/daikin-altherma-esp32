...
// Pull-based signed OTA (esp_http_client -> esp_ota). Flow: check -> lease manifest
// channel/version/ app SHA -> stream and hash daikin-altherma-esp32<suffix>.bin into the inactive
// slot -> release TLS
...
struct OtaStatus {
    ...
    // esp_ota_mark_app_valid_cancel_rollback() succeeded. `unarmed` requires a successful IDF read
    // of a non-pending/non-valid record; blank/no-record otadata is fail-closed `unknown`, not
    // proof that rollback is absent (and rollback_pending is therefore JSON null).
    FixedText<16> image_state; // unknown|pending_verify|valid|unarmed
    // Unknown is not evidence that rollback is absent. HTTP renders rollback_pending as null until
    // the health-gate task has latched an ESP-IDF image-state observation.
    bool rollback_pending_known = false;
...
// When `effective_feed` is non-null, copy the exact two URLs bound to the current successful offer
// as part of the same mutex-owned snapshot.  It is empty before/while a check has no successful
// offer.  Keeping the 512-byte pair out of OtaStatus avoids adding it to every /status/MQTT caller.
OtaStatus ota_status(OtaFeedUrls* effective_feed = nullptr);
