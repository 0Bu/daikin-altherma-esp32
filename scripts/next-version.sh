...
#
# version.txt is the FLOOR: bump it to cut a manual minor/major release. Otherwise the patch
# level auto-increments above the most recent v* tag, so every firmware-relevant change to main
# gets its own monotonic release without a manual version bump.
#
...
if [[ -z "$latest" ]]; then echo "$base"; exit 0; fi
