...
// rollback backup + flags, MQTT, syslog and NTP — are serialized into ONE length-checked,
...
inline constexpr uint8_t  CONFIG_BLOB_VERSION = 1;
...
inline bool config_blob_deserialize(const uint8_t* d, size_t n, ConfigBlob& out) {
    ...
    if (d[4] != CONFIG_BLOB_VERSION) return false;
