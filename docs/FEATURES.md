...
| 20 | **IDF-free host-tested logic core** (2720 checks, re-derived — see §8) | 🧪 | [`main/logic/`](../main/logic), [`test/test_logic.cpp`](../test/test_logic.cpp) |
...
- **✅ 🧪 Second SOURCE: Modbus TCP to a Daikin HomeHub (EKRHH)**
  ...
  **no cost when absent** — an empty address creates no task, socket, discovery or requests. This is
  ...
  WP3 adds exactly one **internal, default-off** actuator below this source path: register 54 through
  the same task-owned socket, with a fixed mailbox, fresh pre-read, bound FC06 echo, FC03 readback and
  restore/conflict state machine. It adds no external control surface; see
  [`MODBUS_ACTUATION.md`](MODBUS_ACTUATION.md). No new ESP-IDF component: `mdns` and `lwip` were already
  linked.
