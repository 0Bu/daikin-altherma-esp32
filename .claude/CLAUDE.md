...
Provenance: the heat-pump protocol/values/converters are ported from
[ESPAltherma](https://github.com/raomin/ESPAltherma) (MIT); the build/installer/captive-portal/
MQTT-HA/OTA/CI scaffolding follows [tesla-key-esp32](https://github.com/0Bu/tesla-key-esp32).
...
since Docker on macOS has no USB passthrough. The `flash-esp32` skill wraps both; the `ship`
skill runs the full delivery (merge PR, follow CI, flash the signed artifact or OTA, verify).
...
wifi.cpp        STA bring-up + scan + mDNS  (reconnect/watchdog: TODO port from tesla-key)
provisioning.cpp captive setup portal (SoftAP daikin-altherma-setup, serves www/setup.html)
hp_comm.cpp     X10A UART (9600 8E1) + queryRegistry — real port of ESPAltherma comm.h
...
def/            embedded value-definition profiles + registry (generated by tools/gen_defs.py)
www/            web UI sources (index.html + style.css + app.js -> one gzipped page) + setup.html
```

**Porting rule:** anything about *talking to the heat pump* comes from ESPAltherma; anything
about *installing/configuring/shipping* comes from tesla-key-esp32. IDF idioms win over Arduino.
Full map: [`docs/ARCHITECTURE.md`](../docs/ARCHITECTURE.md) → ESPAltherma→ESP-IDF porting map.
...
GET  /models      model catalog (indoor/outdoor/tank -> profile, value menu, pin hint)
...
curl http://daikin-altherma.local/status | jq          # device status
curl http://daikin-altherma.local/values | jq          # decoded values
