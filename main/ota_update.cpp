...
// esp_http_client -> esp_ota into the inactive slot) and the ROLLBACK half further down (the health
// gate that keeps a fresh image PENDING_VERIFY until it proves healthy). They are independent
// safety nets and neither replaces the other: the gate below refuses to *start* a bad update, the
// health gate recovers from one that started and booted broken.
//
...
void changelog_lease_expired(TimerHandle_t) {
    ...
    if (s_changelog_expiry_us == 0 && s_changelog_timer) xTimerStop(s_changelog_timer, 0);
}

void ensure_changelog_timer_locked() {
    if (!s_changelog_timer) {
        s_changelog_timer =
            xTimerCreateStatic("ota_notes", kChangelogTimerTickTicks, pdTRUE, nullptr,
                               changelog_lease_expired, &s_changelog_timer_storage);
    }
}
...
struct OtaTaskArgs {
    ...
    // Before a check completes only `offer.feed` is populated.  The successful task fills the
    // remaining fixed identity in this same global busy-owned slot; it then becomes the retained
    // offer lease which an accepted update copies by value back across the task boundary.
    OtaOfferBinding offer{};
};
static_assert(sizeof(OtaTaskArgs) <= 656,
              "OTA task arguments must stay within the reviewed fixed stack allowance");
OtaTaskArgs s_task_args;

// "An OTA network operation is in flight" — the ONE piece of OTA state read from outside on a
// per-second cadence
// (ota_download_active(), see the header for why this is not s_status.state). Deliberately NOT
// under s_mtx: the reader is the MQTT publish task standing aside for exactly the heap event this
// marks, and making it take the lock the OTA task holds — or copy strings to read it — would spend
// the resource it is trying to save. A stale read costs one published cycle either way, so relaxed
...
enum class OtaRuntimeImageState : uint8_t { Unknown, PendingVerify, Valid, Unarmed };
std::atomic<uint8_t> s_runtime_image_state{static_cast<uint8_t>(OtaRuntimeImageState::Unknown)};
static_assert(std::atomic<uint8_t>::is_always_lock_free,
              "OTA image-state snapshot must remain allocation- and lock-free");

const char* ota_runtime_image_state_name(OtaRuntimeImageState state) {
    switch (state) {
    case OtaRuntimeImageState::PendingVerify:
        return "pending_verify";
    case OtaRuntimeImageState::Valid:
        return "valid";
    case OtaRuntimeImageState::Unarmed:
        return "unarmed";
    default:
        return "unknown";
    }
...
// the boot-created socket watchdog below also interrupts IDF's internal header/body receive loops.
...
// opening TLS so a publisher which woke just before the OTA task has time to finish and stand
// aside.
...
const char* ota_transfer_failure_name(OtaTransferFailure failure) {
    switch (failure) {
    case OtaTransferFailure::Read:
        return "read";
    case OtaTransferFailure::Write:
        return "write";
    case OtaTransferFailure::Size:
        return "size";
    case OtaTransferFailure::Hash:
        return "hash";
    case OtaTransferFailure::Timeout:
        return "timeout";
    default:
        return "none";
    }
...
bool set_http_timeout_to_deadline(esp_http_client_handle_t client, TickType_t started,
    ...
    const TickType_t remaining =
        http_deadline_remaining_ticks(started, xTaskGetTickCount(), deadline);
...
bool http_deadline_reached(TickType_t started, TickType_t deadline) {
    return http_deadline_reached_at(started, xTaskGetTickCount(), deadline);
}
...
void close_http_client(esp_http_client_handle_t& client, HttpSocketDeadline& deadline) {
    if (!client) return;
    // Stop or join the socket-shutdown callback before close/cleanup can release and reuse the fd.
...
esp_err_t open_firmware_stream(esp_http_client_handle_t client, OtaFirmwareResponseState& response,
                               HttpSocketDeadline& socket_deadline, int expected_status,
    ...
    for (unsigned redirects = 0; redirects <= kMaxRedirects; ++redirects) {
        ...
        if (e != ESP_OK) {
            return http_deadline_reached(operation_started, operation_deadline) ? ESP_ERR_TIMEOUT
                                                                                : e;
        }
        if (!set_http_timeout_to_deadline(client, operation_started, operation_deadline))
            return ESP_ERR_TIMEOUT;
        ...
        if (arm_err != ESP_OK) return arm_err;
        const int64_t header_result = esp_http_client_fetch_headers(client);
        if (socket_deadline.expired()) return ESP_ERR_TIMEOUT;
        if (header_result < 0) {
            return http_deadline_reached(operation_started, operation_deadline) ? ESP_ERR_TIMEOUT
                                                                                : ESP_FAIL;
        }
        ...
        if (socket_deadline.disarm()) return ESP_ERR_TIMEOUT;
        esp_http_client_close(client);
...
bool wait_for_mqtt_quiesce() {
    ...
    const TickType_t started = xTaskGetTickCount();
    while (!mqtt_publish_network_quiesced() && xTaskGetTickCount() - started < kPollQuiesceWait)
        vTaskDelay(kAllocatorRetryDelay);
    if (mqtt_publish_network_quiesced()) return true;
    const OtaHeapSample sample = ota_heap_sample();
...
bool wait_for_mqtt_transport_quiesce() {
    ...
    const TickType_t started = xTaskGetTickCount();
    while (!mqtt_transport_network_quiesced() && xTaskGetTickCount() - started < kPollQuiesceWait)
        vTaskDelay(kAllocatorRetryDelay);
    if (mqtt_transport_network_quiesced()) return true;
    const OtaHeapSample sample = ota_heap_sample();
...
// Resolve the ordinary CONFIG feed once, at check acceptance, into the same fixed representation as
// an optional HIL feed.  This preserves the UI's live channel switch while making the async task
// independent of later Config reads.  The effective binary base names the selected channel's actual
// directory and always ends in '/', exactly as the eventual image URL does.
void clear_offer_feed_locked() { ota_offer_binding_clear(s_task_args.offer); }
...
bool fetch_manifest_identity_once(const std::string& url, OtaManifestIdentity& out,
    ...
    if (url.empty()) {
        err = "No update URL configured";
        return false;
    }
    if (!ota_url_is_absolute_https(url)) {
        err = "Update URL must use HTTPS";
        return false;
    }
    ...
    if (!wait_for_ota_headroom("manifest", OTA_TRANSFER_HEADROOM, kTransferHeadroomMaxAttempts,
                               manifest_heap)) {
    ...
    if (e != ESP_OK) {
        const HttpClientOpenFailure failure = http_client_log_open_failure("ota", c, e, before);
        // Retry only the allocator-shaped failures seen on the live board. DNS, TCP, certificate
        // and HTTP errors are real reachability failures; retrying those here would merely delay
        // the same answer and blur the diagnostic that distinguishes them.
        retryable_allocator_failure =
            e == ESP_ERR_NO_MEM || failure.tls_error == ESP_ERR_MBEDTLS_SSL_SETUP_FAILED;
        err = retryable_allocator_failure ? "Not enough memory for update TLS — retry after reboot"
                                          : "Can't reach the update server";
    } else {
        const esp_err_t arm_err = socket_deadline.arm(c, manifest_started, kManifestDeadline);
        if (arm_err != ESP_OK) {
            err = arm_err == ESP_ERR_TIMEOUT
                      ? "Update manifest download timed out"
                      : "Update safety timer unavailable — retry after reboot";
        } else {
            const int64_t announced = esp_http_client_fetch_headers(c);
            if (socket_deadline.expired()) {
                ...
                // Read into a FIXED buffer and stop there. Content-Length is a remote claim, so it
                // may never size an allocation; a hostile host advertising 2 GB must simply be
                // refused rather than being believed.
                char          buf[kManifestMax];
                size_t        got          = 0;
                bool          timed_out    = false;
                bool          read_failed  = false;
                bool          reached_eof  = false;
                bool          body_too_big = false;
                const int64_t claimed      = esp_http_client_get_content_length(c);
                if (claimed > static_cast<int64_t>(sizeof(buf))) {
                    err          = "Update manifest is too large";
                    body_too_big = true;
                }
                while (!body_too_big && got < sizeof(buf)) {
                    ...
                    const int n =
                        esp_http_client_read(c, buf + got, static_cast<int>(sizeof(buf) - got));
                    if (socket_deadline.expired()) {
                        err       = "Update manifest download timed out";
                        timed_out = true;
                        break;
                    ...
                    if (n <= 0) {
                        reached_eof = n == 0;
                        if (n < 0) {
                            err         = "Update manifest download failed";
                            read_failed = true;
                        }
                        break;
                    }
                ...
                if (!timed_out && !read_failed && !body_too_big && got == sizeof(buf)) {
                    if (!set_http_timeout_to_deadline(c, manifest_started, kManifestDeadline)) {
                        ...
                        char      extra = 0;
                        const int n     = esp_http_client_read(c, &extra, 1);
                        if (socket_deadline.expired()) {
                            err       = "Update manifest download timed out";
                            timed_out = true;
                        } else if (n < 0) {
                ...
                const bool framed_complete =
                    !timed_out && !read_failed && !body_too_big && reached_eof &&
                    http_body_complete(claimed, got, esp_http_client_is_complete_data_received(c));
                // Join the watchdog before CPU-only parsing and before the client is closed. This
                // also catches a deadline which landed as the final body read returned.
                timed_out = socket_deadline.disarm() || timed_out;
                if (timed_out) {
                    err = "Update manifest download timed out";
                }
                // manifest_identity() is bounded by `got`, assumes no NUL terminator and requires
                // both version and provenance.app_sha256. A legacy/version-only document cannot
                // enter the exact-artifact path.
                if (!timed_out && !read_failed && !body_too_big) {
                    if (got == 0)
                        err = "Empty manifest";
                    else if (!framed_complete)
                        err = "Incomplete update manifest";
...
// Fetch the optional changelog.json published beside the selected manifest.  Failure is
// deliberately non-fatal: a self-hosted or older feed without notes still offers the exact signed
...
ChangelogBuffer fetch_changelog(const std::string& manifest_url, const char* expected_version) {
    ...
    if (!wait_for_ota_headroom("changelog", OTA_CHANGELOG_HEADROOM, kTransferHeadroomMaxAttempts,
                               changelog_heap)) {
    ...
    if (opened == ESP_OK) {
        ...
        if (!set_http_timeout_to_deadline(c, changelog_started, kChangelogDeadline,
            ...
            const esp_err_t arm_err = socket_deadline.arm(c, changelog_started, kChangelogDeadline);
            if (arm_err == ESP_OK) {
                announced = esp_http_client_fetch_headers(c);
                timed_out = socket_deadline.expired();
            } else if (arm_err == ESP_ERR_TIMEOUT) {
                timed_out = true;
            } else {
        ...
        if (!timed_out && !http_deadline_reached(changelog_started, kChangelogDeadline) &&
            deadline_available && announced >= 0 &&
            static_cast<uint64_t>(announced) <= kChangelogDocumentMax &&
            ...
            document.reset(
                static_cast<char*>(heap_caps_calloc(1, kChangelogDocumentMax, MALLOC_CAP_8BIT)));
            if (!document) {
                close_http_client(c, socket_deadline);
                return {};
            }
            ...
            while (got < kChangelogDocumentMax) {
                ...
                const int n = esp_http_client_read(c, document.get() + got,
            ...
            // oversized document whose valid prefix happens to parse could be presented as
            // complete.
            char extra      = 0;
            int  extra_read = 0;
            ...
            timed_out = socket_deadline.expired() || timed_out;
            if (extra_read < 0 && !timed_out) read_error = true;
            const bool complete = extra_read == 0 && esp_http_client_is_complete_data_received(c);
            // Join before CPU-only decoding and before client cleanup. This also classifies a timer
            // callback which landed exactly as the final body read returned.
            timed_out = socket_deadline.disarm() || timed_out;
    ...
    if (!deadline_available)
        diag_printf("ota: skipping changelog because the HTTP safety timer is unavailable\n");
    else if (timed_out)
        diag_printf("ota: changelog download exceeded its absolute deadline\n");
    close_http_client(c, socket_deadline);
    if (!ok) return {};
    ...
    ChangelogBuffer retained(
        static_cast<char*>(heap_caps_malloc(decoded_len + 1, MALLOC_CAP_8BIT)));
    if (!retained) return {};
    std::memcpy(retained.get(), document.get(), decoded_len + 1);
    return retained;
}

void run_check(const OtaTaskArgs& request) {
    set_state("checking");
    const std::string running = esp_app_get_description()->version;
    const OtaChannel  ch      = request.channel;
    const std::string url     = request.offer.feed.manifest.data();
    ...
    diag_printf("ota: %s manifest %s, running %s -> %s\n", ota_channel_name(ch), offer.version,
                ...
                : down ? "older build offered"
                       : "up to date");
    OtaOfferBinding binding{};
    if (!ota_offer_binding_set(binding, request.generation, ota_channel_name(ch), offer.version,
                               offer.app_sha256, request.offer.feed)) {
        diag_printf("ota: check refused an invalid effective feed binding (%s channel)\n",
                    ota_channel_name(ch));
        Lock lk(s_mtx);
        s_status.state            = "error";
        s_status.message          = "No usable update feed configured";
        s_status.update_available = false;
        s_status.downgrade        = false;
        s_status.available.clear();
        s_status.available_sha256.fill('\0');
...
void run_update(const OtaTaskArgs& request) {
    ...
    OtaManifestIdentity offer{};
    const char*         err = "Update check failed";
    if (!fetch_manifest_identity(request.offer.feed.manifest.data(), offer, err)) {
    ...
    if (std::strcmp(offer.version, request.offer.version.data()) != 0 ||
        std::strcmp(offer.app_sha256, request.offer.app_sha256.data()) != 0) {
        diag_printf("ota: update aborted, checked artifact changed (%s channel, expected %s)\n",
                    ota_channel_name(ch), request.offer.version.data());
    ...
    if (!ota_install_allowed(running, avail, allow_downgrade)) {
        ...
        // to land here is an equal (or unparseable) version, i.e. the channel already serves what
        // is running — which is the ordinary outcome of switching to a channel you are already on.
    ...
    const std::string url =
        ota_url_join(request.offer.feed.firmware_base.data(),
    ...
    if (!wait_for_ota_headroom("transfer", OTA_TRANSFER_HEADROOM, kTransferHeadroomMaxAttempts,
                               transfer_heap)) {
    ...
    int                 status           = 0;
    esp_err_t           e = open_firmware_stream(client, response_state, socket_deadline, 200,
                                                 transfer_started, kFirmwareDeadline, status);
    ...
    uint8_t*      buffer =
        static_cast<uint8_t*>(heap_caps_malloc(kOtaBufSize, MALLOC_CAP_8BIT | MALLOC_CAP_INTERNAL));
    if (!buffer) {
        close_http_client(client, socket_deadline);
    ...
    while (probe_len < kImageProbeSize) {
        ...
        if (socket_deadline.expired() ||
            (n <= 0 && http_deadline_reached(transfer_started, kFirmwareDeadline))) {
            header_timed_out = true;
            break;
        }
    ...
    if (probe_len != kImageProbeSize) {
        ...
        heap_caps_free(buffer);
        close_http_client(client, socket_deadline);
        diag_printf("ota: incomplete image header (%u/%u B)\n", static_cast<unsigned>(probe_len),
                    static_cast<unsigned>(kImageProbeSize));
        set_state("error",
                  header_timed_out ? "Update download timed out" : "Update image is unreadable");
        return;
    }
    ...
    while (transfer_ok) {
        ...
        const bool deadline_reached =
            socket_deadline.expired() || http_deadline_reached(transfer_started, kFirmwareDeadline);
        const bool can_resume = total > 0 && ota_transfer_resume_allowed(
                                                 resumes, stream_started_at, written,
                                                 static_cast<uint64_t>(total), deadline_reached);
        if (!can_resume) {
            e = deadline_reached            ? ESP_ERR_TIMEOUT
                : n == -ESP_ERR_HTTP_EAGAIN ? ESP_ERR_HTTP_EAGAIN
                                            : ESP_FAIL;
            transfer_failure =
                deadline_reached ? OtaTransferFailure::Timeout : OtaTransferFailure::Read;
            transfer_ok = false;
            break;
        }
        ...
        if (!wait_for_ota_headroom_until("transfer resume", OTA_TRANSFER_HEADROOM,
            ...
            e = http_deadline_reached(transfer_started, kFirmwareDeadline) ? ESP_ERR_TIMEOUT
                                                                           : ESP_ERR_NO_MEM;
            transfer_failure =
                e == ESP_ERR_TIMEOUT ? OtaTransferFailure::Timeout : OtaTransferFailure::Read;
            transfer_ok = false;
            break;
        }
        ...
        status = 0;
        e = open_firmware_stream(client, response_state, socket_deadline, 206, transfer_started,
        ...
        const bool     range_ok = ota_content_range_matches(response_state.content_range, resume_at,
                                                            static_cast<uint64_t>(total));
        if (!range_ok || esp_http_client_is_chunked_response(client) || response_length < 0 ||
            static_cast<uint64_t>(response_length) != remaining) {
    ...
    if (!transfer_ok || !complete) {
        ...
        diag_printf("ota: download failed (class=%s err=%s, complete=%d, bytes=%u/%lld, "
                    ...
                    complete ? 1 : 0, static_cast<unsigned>(written), static_cast<long long>(total),
                    static_cast<unsigned>(failed.free_bytes),
                    static_cast<unsigned>(failed.largest_internal_block));
        const char* message =
            transfer_failure == OtaTransferFailure::Size ? "Update rejected: image size is invalid"
            : transfer_failure == OtaTransferFailure::Hash
                ? "Couldn't verify the exact update artifact"
            : transfer_failure == OtaTransferFailure::Write   ? "Couldn't write the update"
            : transfer_failure == OtaTransferFailure::Timeout ? "Update download timed out"
    ...
    hash_status =
        psa_hash_finish(&hash_operation, actual_sha256, sizeof(actual_sha256), &actual_sha256_len);
    ...
    if (hash_status != PSA_SUCCESS || actual_sha256_len != sizeof(actual_sha256) ||
        !ota_sha256_matches(actual_sha256, request.offer.app_sha256.data())) {
        if (ota_open) {
            esp_ota_abort(ota_handle);
            ota_open = false;
        }
    ...
    if (e != ESP_OK) {
        ...
        if (e == ESP_ERR_OTA_VALIDATE_FAILED) {
            ...
            // allocation failure into ESP_ERR_OTA_VALIDATE_FAILED.  Do not claim which one
            // occurred.
    ...
    // once more while setting otadata, so protect that second RSA/PSA pass with the same
    // independent post-TLS budget. A refusal still leaves the running slot selected and is safe to
    // retry.
...
uint32_t start_locked(const OtaTaskArgs& request, uint32_t previous_generation) {
    ...
    s_task_args.generation = s_generation;
...
uint32_t start_check(const OtaFeedUrls* hil_feed) {
    ...
    if (hil_feed) {
        if (!ota_feed_urls_valid(*hil_feed)) return 0;
        request.offer.feed = *hil_feed;
    } else if (!ota_default_feed_urls(channel, CONFIG_DAIKIN_OTA_MANIFEST_URL,
                                      CONFIG_DAIKIN_OTA_FIRMWARE_BASE_URL, request.offer.feed)) {
        return 0;
    }

    Lock lk(s_mtx);
    if (s_busy) return 0;
    return start_locked(request, s_generation);
}
...
uint32_t start_update(uint32_t after_generation, const char* expected_channel,
    ...
    if (!after_generation || !expected_channel || !expected_version ||
        !ota_sha256_hex_valid(expected_app_sha256))
        return 0;
    ...
    if (std::strlen(expected_version) == 0 ||
        std::strlen(expected_version) >= s_task_args.offer.version.size())
        return 0;
    ...
    if (s_busy || s_generation != after_generation || s_status.state != "idle" ||
        ...
        (!s_status.update_available && !(allow_downgrade && s_status.downgrade)))
        return 0;
    ...
    OtaFeedUrls bound_feed{};
    if (!ota_offer_binding_copy_feed(s_task_args.offer, after_generation, expected_channel,
                                     expected_version, expected_app_sha256, bound_feed))
        return 0;
    request.offer = s_task_args.offer;
...
uint32_t ota_check_async(int64_t /*browser_epoch_ms*/, const OtaFeedUrls* hil_feed) {
    ...
    const uint32_t generation = start_check(hil_feed);
...
OtaStatus ota_status(OtaFeedUrls* effective_feed) {
    ...
    const auto  image_state =
        static_cast<OtaRuntimeImageState>(s_runtime_image_state.load(std::memory_order_acquire));
    Lock lk(s_mtx);
    s_status.current                      = esp_app_get_description()->version;
    s_status.channel                      = ch;
    s_status.busy                         = s_busy;
    s_status.generation                   = s_generation;
    s_status.heap_min_free_bytes          = s_operation_min_free.load(std::memory_order_relaxed);
    s_status.heap_min_largest_block_bytes = s_operation_min_largest.load(std::memory_order_relaxed);
    s_status.ota_stack_min_free_bytes     = stack_watch_min_free_bytes(StackWatch::Ota);
    s_status.image_state                  = ota_runtime_image_state_name(image_state);
    s_status.rollback_pending_known       = image_state != OtaRuntimeImageState::Unknown;
    s_status.rollback_pending             = image_state == OtaRuntimeImageState::PendingVerify;
    if (effective_feed) {
        if (s_task_args.offer.generation != 0)
            *effective_feed = s_task_args.offer.feed;
        else
            ota_feed_urls_clear(*effective_feed);
    }
    return s_status;
}
...
// Keep rollback armed until this OTA image has proven HEALTHY, not merely survived a timer. A
// normal connected boot must also retain measured internal-heap headroom, report no
// allocation-failure cycles, and — when X10A is live — complete one accepted state publish. The
// dev.13 incident stayed online while persistent publisher buffers fragmented the largest block and
// made /status return 503, so link-only health was a false positive. A provisioning portal remains
// a recovery surface for a fresh/unconfigured board. The decision is the host-tested
// daik::health_gate_decide(); see logic/health_gate.hpp + docs/SECURITY.md.
//
// Only PENDING_VERIFY images are rollback-armed, and those exist ONLY via
// esp_ota_set_boot_partition (a real OTA), which always leaves a valid previous slot.
// Blank/no-state otadata (the normal direct-USB bootstrap case) makes esp_ota_get_state_partition()
// return an error; that is reported fail-closed as Unknown and short-circuits below, so this can
// never strand a fresh board or claim that rollback is absent without an IDF state record.
...
static constexpr int kHealthTaskStack = 4096;  // ELF gate leaves at least 1 KiB after its reviewed
                                               // task/callee/exception-path allowance.
...
static OtaServiceHealth ota_service_health() {
    ...
    return {
        ...
        .allocation_failures =
            heap_guard_restarts() != 0 || mqtt_skips.skipped != 0 || hp_skipped_cycles() != 0,
...
static void health_gate_task(void*) {
    ...
    if (st != ESP_OTA_IMG_PENDING_VERIFY) {
        const OtaRuntimeImageState state =
            st == ESP_OTA_IMG_VALID ? OtaRuntimeImageState::Valid : OtaRuntimeImageState::Unarmed;
        s_runtime_image_state.store(static_cast<uint8_t>(state), std::memory_order_release);
        vTaskDelete(nullptr); // successful IDF observation, but not a rollback-armed image
        return;
    }
    s_runtime_image_state.store(static_cast<uint8_t>(OtaRuntimeImageState::PendingVerify),
                                std::memory_order_release);
    // ONE line per boot rather than one per 5 s cycle: an allocation failure that persists for the
    // whole 600 s cap would otherwise write 120 identical lines into the 6 KB diag ring and evict
    // the boot record this window exists to explain — heap_guard.cpp's throttle, for the same
    // reason.
    ...
    for (int elapsed = 0;; elapsed += kHealthPollS) {
        ...
        // The window this task runs in is the worst possible place to leave that unguarded: 90-600
        // s into an OTA boot, i.e. exactly when the MQTT discovery burst and a TLS session put the
        // heap at its peak. An escape is std::terminate -> reboot, and a reboot while
        // PENDING_VERIFY ROLLS BACK the image — so the failure mode is not "this board reboots" but
        // "a healthy update is reverted on every board that took it", arriving through the update
        // path itself. Skipping a cycle is safe and is the conservative direction: no verdict is
        // reached, elapsed still advances, and the hard cap still ends the window by leaving the
        // image PENDING_VERIFY.
        try {
            const OtaServiceHealth service = ota_service_health();
            const HealthVerdict    v =
                health_gate_decide(elapsed, kHealthBaseWindowS, kHealthHardCapS, service);
            if (v == HealthVerdict::Commit) {
                // The return value decides whether this image survives the next reboot, so it is
                // not one to discard: a failed commit leaves the image PENDING_VERIFY and the
                // bootloader reverts it, and logging the success line over that call would report
                // the opposite of what happened — on the one path where the evidence has to be
                // right.
                const esp_err_t e = esp_ota_mark_app_valid_cancel_rollback();
                if (e == ESP_OK) {
                    s_runtime_image_state.store(static_cast<uint8_t>(OtaRuntimeImageState::Valid),
                                                std::memory_order_release);
            ...
            if (v == HealthVerdict::GiveUp) {
                ESP_LOGW("ota",
                         "health gate: service proof failed after %ds; rebooting while "
                         "PENDING_VERIFY -> rollback to the previous firmware",
                         elapsed);
            ...
            if (!skip_reported) {
                skip_reported = true;
                diag_printf(
                    "ota: health-gate cycle skipped (unknown exception) — still observing\n");
            }
        }
...
// The task no longer copies Config: ota_service_health() reads only atomic/primitive snapshots and
// is currently inlined into this frame. Keep 4096 anyway because the catch path must unwind below
// the throwing frame. The executable ELF gate budgets the task plus 2048 B for IDF/logging,
// exception-unwind and other named-outside-the-component callees, caps that path at 3072 B, and
// thus preserves at least 1 KiB. This transient task deletes itself when the rollback window
// closes.
void ota_health_gate_arm() {
    // If the gate task can't be created, a PENDING_VERIFY OTA image is never marked valid and the
    // bootloader will roll it back on the next reboot — safe, but say so (a silent failure looks
    // like a healthy commit that never happened).
    if (xTaskCreate(health_gate_task, "ota_health", kHealthTaskStack, nullptr, TASK_PRIO_OTA_GATE,
                    nullptr) != pdPASS)
        ESP_LOGE("ota",
                 "health-gate task alloc failed — a pending OTA image will roll back on reboot");
}
