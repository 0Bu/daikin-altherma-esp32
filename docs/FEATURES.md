...
| 50 | **Availability ledger** — is a decoded number a *measurement*, or merely something the firmware could decode? Adjudicated per structural `(page, offset, converter)` key, plus a page-keyed verdict for when the hardware behind a whole register page is not fitted | ✅ 🧪 | [`logic/availability.hpp`](../main/logic/availability.hpp) |
...
| 62 | **Configurable MQTT living-room source** — an exact topic with bounded JSON paths, **test-before-persist** against a single-use mapping-bound proof | ✅ 🧪 | [`logic/reference_temperature.hpp`](../main/logic/reference_temperature.hpp), [`http_config.cpp`](../main/http_config.cpp) |
...
- **✅ 🧪 Manifest check & signed download**: both network operations run on **one on-demand task,
  ...
  for the largest contiguous block). The manifest parser
