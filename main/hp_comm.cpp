...
bool hp_uart_init(int rx, int tx) {
    ...
    // allocated a fresh 256 B RX ring + driver struct + sync primitives ~2x/second and fragmented
    // the heap until an unrelated allocation crashed (a symbolized coredump: hp_poll's std::vector
    // build threw a bad_alloc too starved to unwind -> std::terminate -> abort). So install ONCE,
    // then a genuine pin change is a register-only uart_set_pin re-route that allocates nothing.
    ...
    case UartAction::Remap:
        ...
        // leaves the driver valid on the OLD pins (safer than the old delete-first path, which
        // could strand s_inited==true with no driver).
    ...
    case UartAction::Install: { // first bring-up only
        ...
        // Order matters: set_pin BEFORE install. If the pin config fails (a reserved GPIO can reach
        // the link cache via /set_hp, whose validate() only range-checks — unlike the UI's
        // board_pins filter), NO driver is installed, so s_inited stays false and the detect sweep
        // cleanly falls through to the next candidate pair. Installing first and failing on set_pin
        // would leak the driver AND wedge every later install (ESP_ERR_INVALID_STATE) — the X10A
        // link dead the whole boot.
...
HpQueryResult hp_query_detailed(uint8_t reg, Protocol proto, uint8_t* buf, size_t buflen,
    ...
    // ends in crc_ok(buf, len) reading past the end of the buffer. An out-of-bounds READ that the
    ...
    if (!reply_len_valid(proto, replyLen, buflen)) {
        if (hp_query_should_log(log_policy, HpQueryFailure::InvalidLength))
            diag_printf(
                "HP reply buffer too small for reg 0x%02x: need %d, have %u (rx=%d tx=%d)\n", reg,
                replyLen, static_cast<unsigned>(buflen), s_rx, s_tx);
    ...
    HpFrameReceiver rx(proto, req, qlen, buflen, replyLen);
    int             len   = 0;
    const int64_t   start = esp_timer_get_time();
    while (len < rx.reply_len() && (esp_timer_get_time() - start) < HP_QUERY_TIMEOUT_US) {
        replyLen                   = rx.reply_len();
        const bool header_complete = rx.header_complete(len);
        ...
        uint8_t   ch;
        const int read_res = uart_read_bytes(PORT, &ch, 1, pdMS_TO_TICKS(20));
        if (read_res == 1) {
            rx.feed_byte(ch, buf, len);
            if (rx.has_invalid_length()) {
                if (hp_query_should_log(log_policy, HpQueryFailure::InvalidLength))
                    diag_printf(
                        "HP invalid dynamic reply len %d (max %u) on reg 0x%02x (rx=%d tx=%d)\n",
                        rx.invalid_length(), static_cast<unsigned>(buflen), reg, s_rx, s_tx);
            ...
            if (len == 2 && is_error_reply(buf, len)) {
                if (hp_query_should_log(log_policy, HpQueryFailure::Rejected))
                    diag_printf("HP query rejected for reg 0x%02x (protocol %c, reply 0x15 0xEA; "
                                "rx=%d tx=%d)\n",
