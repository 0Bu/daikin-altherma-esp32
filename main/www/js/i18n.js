...
const I18N = {
  en: {
    ...
    "service.title": "Refrigerant circuit during heating",
    "service.state.waiting": "WAITING FOR HEATING RUN",
    "service.state.observing": "RECORDING",
    "service.state.limited": "RECORDING · SOME DATA MISSING",
    "service.state.interrupted": "PAUSED",
    "service.row.window": "Observed so far", "service.row.reason": "Why this status?",
    "service.reason.unsupported_profile": "This model does not provide every required reading.",
    "service.reason.compressor_not_running": "The compressor is not running.",
    "service.reason.unsupported_or_unknown_mode": "The heat pump is not in ordinary space heating, or its mode is unavailable.",
    "service.reason.dhw_path": "The heat pump is heating domestic hot water.",
    "service.reason.defrost": "The outdoor unit is defrosting.",
    "service.reason.unit_fault": "The heat pump is reporting a fault.",
    "service.reason.special_controller_phase": "A short startup or special controller phase is active.",
    "service.reason.missing_fresh_signal": "At least one required current reading is missing.",
    "service.reason.poll_gap": "The X10A connection was interrupted or intentionally paused.",
    "service.window": (d, n) => `${d} · ${n} current ${n === 1 ? "reading" : "readings"}`,
    "service.help.observing": "Readings are now being recorded continuously during an ordinary heating run.",
    "service.help.limited": "Recording is active, but some additional comparison readings are unavailable.",
    "service.help.interrupted": "Recording ended and restarts automatically during the next suitable heating run.",
    "service.common": "On supported models it starts automatically in ordinary heating; no service mode or setting change. Does not assess refrigerant charge or normal ranges. Valve value: command, not measured position.",
    ...
    "board.led_rgb_wiping": "White, solid — factory reset/data erase in progress; do not disconnect power.",
    ...
    "board.led_gpio_wiping": "Solid after very rapid blinking — factory reset/data erase in progress; do not disconnect power.",
    "board.ledinv": "Active low (LED lights when the pin is driven LOW)",
    "board.btninv": "Active low (button shorts the pin to GND)",
    "board.hint": "Factory reset: hold 5 seconds. Permanently erases Wi-Fi/all settings, recorded history/trends, state durations, and the raw core dump. The setup portal opens only after the full wipe succeeds. If not, release and hold again for 5 seconds. Select “None” if no button is connected.",
