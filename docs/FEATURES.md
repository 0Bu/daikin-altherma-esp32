...
| 20 | **IDF-free host-tested logic core** (1362 checks, re-derived — see §8) | 🧪 | [`main/logic/`](../main/logic), [`test/test_logic.cpp`](../test/test_logic.cpp) |
...
| 36 | **Raw page dump on `/diag`** — the wire bytes of pages `0x00`/`0x10`/`0x20`/`0xA0`/`0xA1`, one line per detect pass. Everything else the device exposes is *decoded*, so an impossible reading cannot be attributed to a wrong converter vs. a wrong offset vs. a per-unit layout difference without these bytes — and they otherwise never leave the board. `0x10`/`0x20` cover impossible readings that fall *inside* the plausibility window and are therefore never masked. **Known limitation** ([#194](https://github.com/0Bu/daikin-altherma-esp32/issues/194)): "one line per detect pass" is also the catch — a detect pass never coincides with a compressor run, so a row that is only wrong *while the unit runs* (`Target Evap. Temp.`) still cannot be caught in the act, and its diagnosis had to back-derive the bytes from the rounded published value | ✅ 🧪 | [`hp_detect.cpp`](../main/hp_detect.cpp), [`logic/hexdump.hpp`](../main/logic/hexdump.hpp) |
