...
const I18N = {
  en: {
    ...
    "dyn.state_waiting": "Waiting for space heating", "dyn.state_room": "Room source unusable",
    "dyn.state_x10a": "X10A offline", "dyn.state_homehub": "HomeHub offline",
    "dyn.state_gate": "Plant state unknown", "dyn.state_clock": "Clock not set",
    "dyn.state_blocked": "Not recording",
    "dyn.state_setup_room": "Set up a room source", "dyn.state_setup_weather": "Set up a location",
    "dyn.state_setup_both": "Not set up yet", "dyn.state_setup_homehub": "HomeHub not set up",
    ...
    "dyn.weather": "Weather forecast", "dyn.strategy": "Method",
    "dyn.not_configured": "Not configured",
    "dyn.shadow_strategy": "P controller, ±2 K",
    "dyn.card_help": "Records, every 30 minutes, the leaving-water offset the weather-dependent curve would need to bring the reference room to its target. Aggregated over a heating season that answers whether the curve is set correctly, and by how much it is not. It runs by itself once a room-temperature source and a location are configured below — there is no switch, because those two sources are what it is. Nothing is ever written to the heat pump; the firmware has no write path.",
    "dyn.state_help_recording": "Space heating is running and the room input is valid, so proposals are being recorded. Read the season verdict from the trend of these proposals, not from a single one.",
    "dyn.state_help_waiting": "The plant is not heating right now, so no proposal is formed — a single proposal needs an active space-heating window. Through the summer this is the normal, expected state and not a fault.",
    "dyn.state_help_blocked": "A required input is missing, so nothing is being recorded. The recording resumes on its own once the input returns; no proposal is invented from stale data.",
    "dyn.state_help_room": "The room reading is reaching the device, but in this state it cannot say how far the room is from its target — so no proposal is formed from it. Nothing else is wrong; recording resumes by itself once the source becomes usable again.",
    "dyn.state_help_setup": "The diagnosis runs as soon as both of its inputs exist: a room-temperature source over MQTT and a location for the forecast. Configure them in the rows below — there is nothing else to switch on.",
    ...
    "dyn.strategy_help": "Every 30 minutes at most, a P controller turns the room deviation into a whole-kelvin proposal: 0.25 K deadband, limited to ±2 K overall and 1 K per step. The proposal is a MEASUREMENT — it is recorded and never applied.",
    ...
    "wx.title": "Open-Meteo weather forecast", "wx.latitude": "Latitude", "wx.longitude": "Longitude", "wx.error": "Open-Meteo forecast error",
  ...
  de: {
    ...
    "dyn.state_waiting": "Wartet auf Heizbetrieb", "dyn.state_room": "Raumquelle nicht verwertbar",
    "dyn.state_setup_room": "Raumquelle einrichten", "dyn.state_setup_weather": "Standort einrichten",
    "dyn.state_setup_both": "Noch nicht eingerichtet", "dyn.state_setup_homehub": "HomeHub nicht eingerichtet",
    ...
    "dyn.state_x10a": "X10A offline", "dyn.state_homehub": "HomeHub offline",
    "dyn.state_gate": "Anlagenzustand unbekannt", "dyn.state_clock": "Uhrzeit nicht gesetzt",
    "dyn.state_blocked": "Zeichnet nicht auf",
    "dyn.room_source": "Raumtemperaturquelle", "dyn.configured": "Konfiguriert",
    "dyn.weather": "Wetterprognose", "dyn.strategy": "Verfahren",
    "dyn.not_configured": "Nicht konfiguriert",
    "dyn.shadow_strategy": "P-Regler, ±2 K",
    "dyn.card_help": "Zeichnet alle 30 Minuten auf, welchen Vorlauf-Offset die witterungsgeführte Heizkurve bräuchte, um den Referenzraum auf seinen Sollwert zu bringen. Über eine Heizsaison aggregiert beantwortet das, ob die Kurve richtig eingestellt ist und um wie viel nicht. Sie läuft von selbst, sobald unten eine Raumtemperaturquelle und ein Standort konfiguriert sind — es gibt keinen Schalter dafür, denn diese beiden Quellen SIND die Diagnose. An die Wärmepumpe wird nichts geschrieben; die Firmware hat keinen Schreibpfad.",
    "dyn.state_help_recording": "Der Raumheizbetrieb läuft und der Raumeingang ist gültig, es werden also Vorschläge aufgezeichnet. Der Saisonbefund ergibt sich aus dem Verlauf dieser Vorschläge, nicht aus einem einzelnen.",
    "dyn.state_help_waiting": "Die Anlage heizt gerade nicht, deshalb entsteht kein Vorschlag — dafür braucht es aktiven Raumheizbetrieb. Über den Sommer ist das der normale, erwartete Zustand und kein Fehler.",
    "dyn.state_help_blocked": "Ein benötigter Eingang fehlt, es wird nichts aufgezeichnet. Sobald er zurückkommt, läuft die Aufzeichnung von selbst weiter; aus veralteten Daten wird kein Vorschlag erfunden.",
    "dyn.state_help_room": "Der Raumwert erreicht das Gerät, kann in diesem Zustand aber nicht sagen, wie weit der Raum von seinem Sollwert entfernt ist — daraus entsteht also kein Vorschlag. Sonst ist nichts verkehrt; sobald die Quelle wieder verwertbar ist, läuft die Aufzeichnung von selbst weiter.",
    "dyn.state_help_setup": "Die Diagnose läuft, sobald ihre beiden Eingänge vorhanden sind: eine Raumtemperaturquelle über MQTT und ein Standort für die Prognose. Beide in den Zeilen darunter einrichten — sonst ist nichts einzuschalten.",
    ...
    "dyn.strategy_help": "Höchstens alle 30 Minuten macht ein P-Regler aus der Raumabweichung einen ganzzahligen Vorschlag: 0,25 K Totzone, insgesamt auf ±2 K und je Schritt auf 1 K begrenzt. Der Vorschlag ist eine MESSGRÖSSE — er wird aufgezeichnet und nie angewendet.",
    ...
    "wx.title": "Open-Meteo-Wetterprognose", "wx.latitude": "Breitengrad", "wx.longitude": "Längengrad", "wx.error": "Fehler Open-Meteo-Prognose",
