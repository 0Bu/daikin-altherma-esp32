...
static const uint8_t PROBE_PAGES_I[] = {0x00, 0x10, 0x11, 0x20, 0x21, 0x30, 0x60,
                                        0x61, 0x62, 0x63, 0x64, 0x65, 0xA0, 0xA1};
...
static bool proto_answers(Protocol p) {
    uint8_t tmp[64];
    return read_page(p == Protocol::S ? 0x50 : 0x00, p, tmp, sizeof(tmp)) >= 0;
}
...
// then answered. Attempts spent on a page the unit simply does not have are NOT counted: every
// probe sweep tries pages no single model carries (0x65 answers on none of the measured units), so
...
DetectResult hp_detect_run() {
    ...
    //    tried FIRST), their swap (a swapped X10A wire is the most common mistake), then the
    //    per-target Kconfig default and its swap — so a stale/default cache still self-heals to the
    //    real wiring. All pins are X10A-designated (no arbitrary GPIO probing). Keep the framing +
    //    pins that answer.
    struct PinPair {
        int rx, tx;
    };
    ...
    auto    add = [&](int rx, int tx) {
        ...
        for (int i = 0; i < nc; i++)
            if (cand[i].rx == rx && cand[i].tx == tx) return;
    ...
    for (int i = 0; i < nc && !r.bus_ok; i++) {
        diag_printf("detect: probing rx=%d tx=%d (proto %c/%c)\n", cand[i].rx, cand[i].tx,
                    static_cast<char>(p0), static_cast<char>(p1));
        // CHECK the bring-up, exactly as poll_once does, and for two distinct reasons:
        //   * A failed REMAP leaves the driver valid on the PREVIOUS pins (hp_comm.cpp keeps it
        //   that
        //     way on purpose). Probing on anyway means an answer heard on the old pair gets
        //     recorded as this candidate's — and poll_detect would offer that wrong pair to the
        //     generation/revision-checked detected-link commit, so a later boot could start from a
        //     lie the sweep has to undo.
        //   * A failed first INSTALL leaves no driver at all. Probing anyway would make a local
        //   UART
    ...
    if (!r.bus_ok) {
        if (nc > 0) {
            diag_printf("detect: no X10A response on any pin/proto (last tried rx=%d tx=%d) — "
                        "check wiring/GND\n",
    ...
    int            len60 = -1;
    uint8_t        page10[32];
    int            len10 = -1; // dump-only (below): target temps
    uint8_t        page20[32];
    int            len20 = -1; // dump-only (below): O/U sensors + pressures
    uint8_t        pageA0[32];
    int            lenA0 = -1; // O/U-II rows — raw, for the diag dump below
    uint8_t        pageA1[32];
    int            lenA1                  = -1;
    ...
    for (size_t pi = 0; pi < num_pages; ++pi) {
        ...
        if (reg == 0x11) {
            len11 = paylen;
            for (int i = 0; i < paylen && i < static_cast<int>(sizeof(page11)); i++)
                page11[i] = pay[i];
        ...
        if (reg == 0x00) {
            len00 = paylen;
            for (int i = 0; i < paylen && i < static_cast<int>(sizeof(page00)); i++)
                page00[i] = pay[i];
        } else if (reg == 0x60) {
            len60 = paylen;
            for (int i = 0; i < paylen && i < static_cast<int>(sizeof(page60)); i++)
                page60[i] = pay[i];
        } else if (reg == 0x10) {
            len10 = paylen;
            for (int i = 0; i < paylen && i < static_cast<int>(sizeof(page10)); i++)
                page10[i] = pay[i];
        } else if (reg == 0x20) {
            len20 = paylen;
            for (int i = 0; i < paylen && i < static_cast<int>(sizeof(page20)); i++)
                page20[i] = pay[i];
        } else if (reg == 0xA0) {
            lenA0 = paylen;
            for (int i = 0; i < paylen && i < static_cast<int>(sizeof(pageA0)); i++)
                pageA0[i] = pay[i];
        } else if (reg == 0xA1) {
            lenA1 = paylen;
            for (int i = 0; i < paylen && i < static_cast<int>(sizeof(pageA1)); i++)
                pageA1[i] = pay[i];
        }
    }
    ...
    //             25-40 bar there. Absent sensor or wrong offset — the bytes decide, the decode
    //             can't.
    //     One line per page, only on a detect pass (not per poll cycle), so the diag ring (6 KB)
    //     and syslog stay readable: 5 lines of ~120 chars against a 256-byte line buffer.
    ...
    if (r.proto == Protocol::S) {
        ...
        const char*      out[64];
        const int        n =
            detect_candidates(sigs, nsig, fp, out, static_cast<int>(sizeof(out) / sizeof(out[0])));
    ...
    diag_printf("detect: proto=%c rx=%d tx=%d pages=0x%04x kw=%d iu_kw=%d eeprom=[%s] retries=%d "
                ...
                fp.kw_tenths, fp.iu_kw_tenths, ee, probe_retries, probe_transport_errors,
                static_cast<int>(r.candidates.size()), r.best.empty() ? "(none)" : r.best.c_str());
    return r;
}
