...
| 64 | **Direct Open-Meteo forecast on the ESP32.** Configured latitude/longitude selects one location; MQTT, evcc and k3s are not dependencies. A dedicated task waits for WiFi and synchronized time, then fetches a bounded six-hour JSON response over CA-verified HTTPS every 45 minutes with a 5-minute error retry. The query pins `icon_seamless`, asks only for `temperature_2m` and `shortwave_radiation`, uses Unix/GMT timestamps and validates both units before deriving the next two complete hours' mean °C and Wh/m². Fetch/decision times, explicit unavailable provider issue time, location, model, availability, freshness and counters are exposed in `/status`; coordinates are redacted in diagnostic snapshots. If MQTT and Open-Meteo are configured, `<base>/weather/openmeteo/forecast` retains one atomic evidence snapshot containing the derived values, provider/model, fetch/forecast/valid-until times and fail-closed availability/freshness flags; it deliberately contains no coordinates and creates no HA Discovery entities. Without Open-Meteo configuration no state document is published; clearing both location fields queues one QoS-1 retained empty tombstone after the disabled state is persisted, so the broker definitively removes the topic even without X10A. Upgrade cleanup retracts the four weather configs published by the earlier short-lived contract and removes its retained `<base>/weather_forecast` predecessor when present. Failed refreshes keep the old figures visible only for diagnosis while `available`/`fresh` fail closed. Blob v10 persists signed microdegrees plus an explicit enable bit, both empty UI fields disable all weather traffic, and `POST /set_weather` only saves/wakes so network work never blocks httpd. A latitude/longitude pair copied from Google Maps can be pasted into either coordinate field; the UI splits, range-checks and rounds it to the six decimal places persisted by the firmware. Nothing is persisted or sent to Open-Meteo before Save. The modal discloses location/IP transfer and attributes Open-Meteo/DWD ICON. The public endpoint is intended for non-commercial use and has no uptime guarantee; the provider response is not stored or backfilled and no forecast is used for heat-pump writes yet | ✅ 🧪 | [`weather_forecast.cpp`](../main/weather_forecast.cpp), [`logic/open_meteo.hpp`](../main/logic/open_meteo.hpp), [`logic/weather_forecast.hpp`](../main/logic/weather_forecast.hpp), [`logic/weather_mqtt.hpp`](../main/logic/weather_mqtt.hpp), [`http_config.cpp`](../main/http_config.cpp), [`http_status.cpp`](../main/http_status.cpp), [`www/js/settings.js`](../main/www/js/settings.js) |
