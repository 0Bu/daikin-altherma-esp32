...
- **✅ 🧪 HA MQTT auto-discovery.** One retained discovery config per value of the active profile
  ...
  topic. The former retained `<base>/modbus/status` is deleted because heartbeat already carries link
  state, RX and failures.
  Its Int16 enums remain numeric constants on MQTT (mode 2 stays `2`, never `"Recommended on"`); the
  browser names them from separate semantic metadata. A bounded
