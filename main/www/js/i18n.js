...
const I18N = {
  en: {
    ...
    "hist.boost_run": (when, d) => `${when} · approx. ${d}`,
    ...
    "hist.boost_aria": (l, d) => `${l} — Boost timeline. ${d}. Arrow keys read individual samples.`,
    "hist.heater_total": (d) => `Heater observed active · ${d} sampled raster time`,
    "hist.heater_run": (when, d) => `${when} · active window approx. ${d}`,
    ...
    "hist.buh_run": (stage, when, d) => `${stage} · ${when} · active window approx. ${d}`,
    ...
    "hist.valve_run": (when, d) => `${when} · DHW approx. ${d}`,
    ...
    "dyn.weather": "Weather forecast", "dyn.strategy": "Control strategy",
    ...
    "ref.test": "Test", "ref.testing": "Waiting for value…",
    "ref.test_passed": (value, retained) => `Read ${value} °C${retained ? " (retained)" : ""}. Saving is now enabled.`,
    "ref.test_failed": "No readable, fresh MQTT value received",
    "ref.test_required": "Test this MQTT mapping successfully before saving",
    ...
    "ref.hint": "This stage supports one MQTT source in Observe mode. Test must read a fresh value before Save is enabled. The optional timestamp may be RFC3339 text or Unix seconds. Retained values require it to be fresh; live values without it age from MQTT arrival. Leave the topic empty to disable capture. The Shelly example is its internal device temperature and is only a transport test.",
  ...
  de: {
    ...
    "hist.boost_run": (when, d) => `${when} · ca. ${d}`,
    ...
    "hist.boost_aria": (l, d) => `${l} — Boost-Verlauf. ${d}. Mit den Pfeiltasten einzelne Zeitpunkte ablesen.`,
    "hist.heater_total": (d) => `Heizstab aktiv erfasst · ${d} Rasterzeit`,
    "hist.heater_run": (when, d) => `${when} · aktives Zeitfenster ca. ${d}`,
    ...
    "hist.buh_run": (stage, when, d) => `${stage} · ${when} · aktives Zeitfenster ca. ${d}`,
    ...
    "hist.valve_run": (when, d) => `${when} · Warmwasser ca. ${d}`,
    ...
    "dyn.weather": "Wetterprognose", "dyn.strategy": "Regelstrategie",
    ...
    "ref.test": "Testen", "ref.testing": "Warte auf Wert…",
    "ref.test_passed": (value, retained) => `${value} °C gelesen${retained ? " (retained)" : ""}. Speichern ist jetzt möglich.`,
    "ref.test_failed": "Kein lesbarer, frischer MQTT-Wert empfangen",
    "ref.test_required": "Diese MQTT-Zuordnung vor dem Speichern erfolgreich testen",
    ...
    "ref.hint": "Diese Stufe unterstützt eine MQTT-Quelle im Modus Beobachten. Erst ein erfolgreich gelesener, frischer Testwert schaltet Speichern frei. Der optionale Zeitstempel darf RFC3339-Text oder Unix-Sekunden sein. Retained Werte benötigen ihn, um frisch zu sein; Live-Werte ohne ihn altern ab MQTT-Empfang. Ein leeres Topic deaktiviert die Erfassung. Das Shelly-Beispiel ist dessen interne Gerätetemperatur und dient nur als Transporttest.",
    "syslog.title": "Syslog-Server", "syslog.hostport": "Host : Port",
