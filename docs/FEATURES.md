...
| 42 | **24-hour trend rings** — a fixed-cadence (5 min × 288) `int16` ring per trended value or state, **static** rather than on the heap (576 B each; **19 X10A/board/state rings = 10944 B** plus **seven label-free HomeHub rings = 4032 B**, **14976 B total**, guarded by separate ceiling asserts). Their non-zero `pending` sentinel places the rings in `.data`, not `.bss`, so each also costs its size in the flash image. Chunked `GET /history` serves X10A by default and `source=modbus` for the second instrument; the UI draws the result in the value-row explainer and schematic inspector. Six structurally paired measurements overlay blue X10A and petrol HomeHub lines on one aligned scale. The seventh HomeHub ring and nineteenth primary ring carry the four-state Smart-Grid mode: the browser renders them as separate state tracks, marks mode 2 as Boost, and lists sampled Boost intervals plus total duration instead of drawing a misleading numeric 0–3 curve. Two X10A absences remain distinguished: a register that did not answer vs. an outdoor-unit page held over while the compressor rests. The four computed pills (ΔT, heat output, electrical input, COP) still have no ring of their own; `www/js/history.js` assembles those curves from the source rings using the live expressions. Two primary trends are the board's own **free heap** and **largest contiguous block**. Row trends are addressed structurally by **(register page, byte offset, unit)**, while the Smart-Grid trend combines the two structurally identified contact bits. HomeHub history membership is explicit in `HOMEHUB_HISTORIES`; compile-time guards pin it to real trends and unique offsets | ✅ 🧪 | [`logic/history.hpp`](../main/logic/history.hpp), [`logic/homehub_map.hpp`](../main/logic/homehub_map.hpp), [`history.cpp`](../main/history.cpp), [`http_status.cpp`](../main/http_status.cpp), [`www/js/history.js`](../main/www/js/history.js) |
