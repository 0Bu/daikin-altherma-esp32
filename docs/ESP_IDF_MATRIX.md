...
The CI image selects **ESP-IDF 6.1**, the dependency lock resolves exact **ESP-IDF 6.1.0**, and the
project targets **ESP32-S3**. The general entry point requested for this inventory is Espressif's
[ESP32 Get Started guide][idf-esp32-start]; feature rows use the corresponding [ESP32-S3 6.1 API
reference][idf-s3-api] because ESP32-S3 is the only supported target. Espressif publishes the 6.1.0
release documentation under its `v6.1` channel. Official components extracted from ESP-IDF are
linked to the Espressif Component Registry.

<!-- esp-idf-matrix:metadata {"target":"esp32s3","version":"6.1.0","idf_floor":">=6.0"} -->
...
| U15 | Multicast DNS | ✅ Direct + 📦 Managed | `mdns`, `espressif/mdns` | Device HTTP service advertisement and bounded one-shot HomeHub discovery; the unused interactive console and continuous browse daemon are omitted | [`net.cpp`](../main/net.cpp), [`hp_modbus.cpp`](../main/hp_modbus.cpp), [`sdkconfig.defaults`](../sdkconfig.defaults) | [mDNS component](https://components.espressif.com/components/espressif/mdns) |
...
| N25 | Internal Ethernet EMAC | ⛔ Unsupported | Not present on ESP32-S3 | Wired networking correctly uses an external W5500 over SPI | [`net.cpp`](../main/net.cpp), [`BOARDS.md`](BOARDS.md) | [Ethernet](https://docs.espressif.com/projects/esp-idf/en/v6.1/esp32s3/api-reference/network/esp_eth.html) |
...
| `CONFIG_LWIP_TCP_SND_BUF_DEFAULT` | `5760` | U10 |
...
| `CONFIG_ESP_WIFI_ENABLE_WPA3_OWE_SOFTAP` | `n` | U08 |
| `CONFIG_ESP_WIFI_ENABLE_SAE_PK` | `n` | U08 |
| `CONFIG_ESP_WIFI_SOFTAP_SAE_SUPPORT` | `n` | U08 |
| `CONFIG_MDNS_ENABLE_CONSOLE_CLI` | `n` | U15 |
| `CONFIG_MDNS_ENABLE_BROWSE` | `n` | U15 |
...
[idf-s3-api]: https://docs.espressif.com/projects/esp-idf/en/v6.1/esp32s3/api-reference/index.html
