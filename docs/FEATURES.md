...
| 90 | **Bounded X10A publish path + weather-fetch headroom gate** — a source-tagged, profile-stable cache/group layout and static fixed-capacity 12 KiB JSON sink eliminate firmware-owned steady-state growth and the large payload heap block (missing/held rows toggle presence; mismatched/oversized snapshots fail closed); FNV-1a dedup advances only after an accepted broker write and the OOM catch records the throw-second heap snapshot; Open-Meteo waits for X10A, firmware MQTT publish-cycle and esp-mqtt connect/reconnect quiescence, probes after URL construction and retries in 5 min below 56 KiB free / 24 KiB largest contiguous block | ✅ 🧪 | [`mqtt_ha.cpp`](../main/mqtt_ha.cpp), [`hp_poll.cpp`](../main/hp_poll.cpp), [`logic/x10a_snapshot.hpp`](../main/logic/x10a_snapshot.hpp), [`logic/mqtt_group.hpp`](../main/logic/mqtt_group.hpp), [`logic/json.hpp`](../main/logic/json.hpp), [`weather_forecast.cpp`](../main/weather_forecast.cpp), [`logic/weather_forecast.hpp`](../main/logic/weather_forecast.hpp), [`test_x10a_publish_heap_contract.mjs`](../test/test_x10a_publish_heap_contract.mjs) |
