...
// ── The claim the device made is the claim that is printed ──────────────────────────────────────
{
  ...
  assert.match(a.dwellNoteHtml(ROW({ dwell_s: 12000 }), "OFF"), /OFF for 3 h 20 min/,
    "a witnessed run states its age");
  assert.doesNotMatch(a.dwellNoteHtml(ROW({ dwell_s: 12000 }), "OFF"), /at least/,
  ...
  assert.match(a.dwellNoteHtml(ROW({ dwell_s: 0 }), "ON"), /ON for 0 s/,
  ...
  const gapped = a.dwellNoteHtml(ROW({ dwell_s: 12000, dwell_blind_s: 600 }), "OFF");
  assert.match(gapped, /10 min of it not observed/, "material blind time must be stated");
  assert.match(a.dwellNoteHtml(ROW({ dwell_s: 12000, dwell_blind_s: 5 }), "OFF"),
    /5 s of it not observed/, "a small gap is still unobserved time and must be stated");
  assert.doesNotMatch(a.dwellNoteHtml(ROW({ dwell_s: 12000 }), "OFF"), /observed/,
...
// ── In the row ──────────────────────────────────────────────────────────────────────────────────
{
  ...
  assert.match(html, /OFF for 3 h 20 min/, "the age must reach the panel");
  ...
  assert.ok(html.indexOf("OFF for 3 h 20 min") < html.indexOf("Powerful operation starts tank heating"),
  ...
  assert.match(dead, /—/, "a silent bus blanks the reading");
  assert.doesNotMatch(dead, /for 3 h 20 min/,
  ...
  assert.doesNotMatch(nulled, /for 3 h 20 min/,
...
// trend the firmware does not buffer would otherwise be a plain row silently dropping its age.
{
  ...
  assert.match(a.vDescRow(orphan), /ON for 5 min/);
