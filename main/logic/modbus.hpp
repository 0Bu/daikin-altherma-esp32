...
// Per-request read maximum (Modbus application protocol §6.3). A read response carries its payload
...
// ── Response parsing ────────────────────────────────────────────────────────────────────────────
enum class MbParse {
    Ok,            // well-formed, request-bound read response (`payload` contains register data)
...
// request. `expect_qty` is the register quantity requested; the reply must carry exactly that many
// values (QtyMismatch otherwise). A Modbus read reply does not echo its start address, so the
// transaction id is what binds the response to its request.
inline MbParse mb_parse_response(const uint8_t* adu, int adu_len, uint16_t expect_txn,
                                 uint8_t expect_unit, MbFunc expect_fc,
                                 uint16_t expect_qty, MbResponse& out) {
    out = MbResponse{};
    ...
    if (bytecount / 2 != static_cast<int>(expect_qty)) return MbParse::QtyMismatch;
    out.ok = true;
    return MbParse::Ok;
}
