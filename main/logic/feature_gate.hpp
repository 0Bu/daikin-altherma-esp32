#pragma once
// WHICH DERIVED FEATURES MAY HONESTLY RUN on the detected model — the policy half of issue
// legacy-69 step 0.2 (spun off as legacy-110 Part C): what UC5 ("silent protection retries" early
// warning) and, later, on-device inference do when the active profile does not carry the signals
// they are defined on.
//
...
//     (legacy-121); when the outdoor unit stops refreshing its own pages the pills blank rather
//     than showing a dimmer register of half-valid numbers (logic/ou_stale.hpp, and v1.0.13's
...
// legacy-69 frames this as a `generic`-fallback problem, and `generic` IS the extreme case:
// measured on this catalog it carries no leaving-water measurement (only "LW setpoint (main)",
// which lwt_select() correctly rejects), no INV frequency, no expansion valve and no pressure row
// at all. But it is not the only case. Measured across the 43 generated profiles, SIXTEEN lack
// register page 0x30 and with it both "INV frequency (rps)" and the expansion-valve positions. An
// id check on "generic" would therefore have let inference run without a run-state input on more
// than a third of the DETECTED catalog — the same defect the check was written to prevent, on a
// profile nobody thought to look at. Coverage is a property of the row set, so it is read off the
// row set.
//
// Pure and host-tested for the usual reason: there is no firmware caller yet (legacy-69 Phase 3 has
// not landed), and a policy that is only prose gets re-litigated at the call site. The test pins
// the measurements above against the whole catalog, so a generator run that adds or removes a page
...
// www/js/schematic.js's `vNum(/inv frequency/i)` (see logic/ou_stale.hpp for why this row in
// particular has to stay trustworthy: it is what makes "Standby — not running" believable while the
// pills around it are held over). One rule, two languages, same tokens.
...
// wrong because every detected hydronic profile also carries Water pressure in bar.  The
// converter's shared structural predicate owns the physical rule; this adapter supplies the
// same-register conv-405 saturation twin across all view spans without flattening them into a heap
// allocation.
...
inline FeatureCoverage feature_coverage(const ProfileView& v) {
    ...
    for (size_t i = 0; i < v.count(); i++) {
        ...
        // pattern is the documented way legacy-121 re-opens; there must not be one.
...
inline bool uc5_supported(const FeatureCoverage& c) { return c.retry_counters && c.run_state; }

// The later inference use cases (UC1 leak/anomaly, UC2 heating curve) add the hydronic working
// point and the refrigerant-circuit signals. Strictly stronger than uc5_supported() — a build that
// can run a trained model can always run the rule.
