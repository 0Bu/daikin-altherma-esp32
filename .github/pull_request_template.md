<!--
Fill in each section. Delete checklist lines that don't apply. This template is used by human
authors and by multiple coding agents — keep it honest: say what was actually verified, and note
anything that couldn't be (a remote session may have no Docker daemon or USB — see AGENTS.md).

FIRST-TIME CONTRIBUTOR? Read CONTRIBUTING.md. Short version: run the scripts under
"Verification", say honestly what you could not test, and leave the "Maintainer gates" section
alone — those are repository-maintainer skills and are not yours to run.
-->

## Summary

<!-- What changed and why, in 1-3 sentences. -->

## Changes

-

## Verification

- [ ] `scripts/run-mock-tests.sh` passes (host-side logic tests — a CI `gates` step)
- [ ] `scripts/run-domain-audit.sh` clean (value-catalog domain correctness — a CI `gates` step)
- [ ] `scripts/run-description-audit.sh` clean (every visible reading has an explainer)
- [ ] `scripts/run-schematic-audit.sh` clean (the dashboard drawing — only if the web UI changed)
- [ ] `scripts/run-ui-use-case-tests.sh` passes (all UI actions and modal lifecycles — a CI `gates` step)
- [ ] `tools/absence/selftest.sh` passes (the source-absence matrix can still go red — a CI `gates` step; the matrix itself rides the contract + UI suites)
- [ ] `scripts/run-agent-instructions-budget.sh` passes (agent mapping, parity and instruction budgets)
- [ ] Firmware built (`scripts/idf-docker.sh idf.py build`, or relied on CI) — N/A in a cloud session (no Docker daemon / no USB)
- [ ] Exercised against a real heat pump / device where relevant (or noted why not)

## Checklist

- [ ] Docs kept in sync where behaviour changed (`AGENTS.md`, `docs/ARCHITECTURE.md`, `README.md`, `docs/README.md`, `docs/SECURITY.md`; Claude compatibility guidance aligned where agent rules changed)
- [ ] New hardware-free logic lives in `main/logic/` with a `CHECK` in `test/test_logic.cpp` (see `$add-logic-test`)
- [ ] Value/CRC/protocol changes match a verified reference; generated `def/*` profiles were regenerated by the offline decode tooling (never hand-edited) and their rows check out against `docs/REGISTERS.md`
- [ ] Heap-safe — no new large *contiguous* allocations; HTTP handlers stay under the OOM try/catch
- [ ] Target-specific — builds for esp32s3

## Maintainer gates

<!-- Not runnable by outside contributors — leave unchecked, the maintainer fills these in.
     `$name` is canonical; Claude's legacy `/name` spelling remains accepted during migration.
     CI validates exact syntax and current-head freshness, not the identity of the body editor;
     maintainer review plus GitHub merge authorization is the actor trust boundary. -->

- [ ] `$project-review` clean — merge gate @ <short-sha> (required on every merge: doc drift, memory safety, tests and target/build implications)
- [ ] `$feature-docs` synced — merge gate @ <short-sha> (required if a technical feature changed: `docs/FEATURES.md` matches the code)
- [ ] `$domain-review` clean — merge gate @ <short-sha> (required on every merge: values verified physically right, sensible and authentic; a PR that cannot reach a value clears quickly, but say what you checked)
- [ ] `$schematic-review` clean — merge gate @ <short-sha> (required if the dashboard schematic changed: the drawing still tells the truth about the plant, and every reading is on the part that measures it)
- [ ] `$ui-use-case-review` clean — merge gate @ <short-sha> (required if UI behavior changed)
- [ ] `$absence-review` clean — merge gate @ <short-sha> (required if an OPTIONAL SOURCE changed: the broker, the room source, the circulation witness, the HomeHub, ENV III, the weather location, the X10A bus, safe mode, or anything that reports one)
- [ ] `$ui-gif` clean — merge gate @ <short-sha> (required when this PR re-made `docs/media/dashboard.gif` or its stamp; a red `scripts/run-ui-gif-audit.sh` is a hard block that only a real re-record can clear)
<!-- The stamp must be a BARE sha after the `@` — the gate matches `@[[:space:]]*[0-9a-f]{7,40}`
     (tools/agent-hooks/pr-gate-lib.sh; .claude keeps compatibility wrappers), so wrapping it in
     backticks parses as no stamp at all and the
     merge is refused with "ticked but carries no @ <sha> stamp". Get it with
     `git rev-parse --short=12 HEAD`. -->

<!-- `$ui-gif` needs Chrome + ffmpeg, so it is the one gate an outside contributor cannot clear. If
     that is you: leave it unchecked and say so — the maintainer re-records. What nobody may do is
     rewrite `tools/uigif/gif_stamp.txt` to make CI green; the stamp writer refuses that shape, and
     it is the one move that turns this gate into a lie. Canonical `$name` and retained Claude
     compatibility `/name` records are both accepted during migration. -->
