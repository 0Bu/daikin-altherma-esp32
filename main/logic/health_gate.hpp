...
// to make that call, so a boots-but-broken update (e.g. a WiFi regression that never gets online and
// so can never be re-flashed OTA) is left un-committed and reverts on the next reboot, instead of
...
//   wifi_configured  device has STA credentials (so connectivity IS the expected healthy state)
//   wifi_connected   STA currently has an IP
//
// "healthy" = connectivity proven (STA got an IP) OR the device is legitimately in setup mode (no
// credentials -> the SoftAP portal, which never reaches wifi_connected, is a valid running state and
// must not be treated as broken). A device with credentials that cannot get online within hard_cap_s
// is treated as broken and is NOT committed.
inline HealthVerdict health_gate_decide(int elapsed_s, int base_window_s, int hard_cap_s,
                                        bool wifi_configured, bool wifi_connected) {
    const bool healthy = wifi_connected || !wifi_configured;
