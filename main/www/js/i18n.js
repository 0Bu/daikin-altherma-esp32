...
const I18N = {
  en: {
    ...
    "dyn.card": "Dynamic leaving-water control", "dyn.experimental": "Experimental",
    "dyn.mode": "Operating mode", "dyn.observe": "Observe", "dyn.off": "Off",
    ...
    "dyn.shadow_strategy": "P controller · shadow",
    "dyn.enable": "Dynamic leaving-water control",
    "dyn.enable_help": "Off by default. Enabling starts the experimental shadow analysis and its configured MQTT and weather inputs. A configured MQTT room source with target and timestamp, MQTT broker and HomeHub are required. Shadow mode never writes a correction to the heat pump.",
    "dyn.enabled": "Experimental shadow analysis enabled",
    "dyn.disabled": "Dynamic leaving-water control disabled",
    "dyn.enable_requirements": "Configure MQTT, a complete room-temperature source and HomeHub before enabling.",
    "dyn.mode_help": "Observe subscribes to the configured room input, fetches weather data and evaluates the experimental controller. It does not change the heat pump or its leaving-water target.",
    "dyn.off_help": "The feature is off. Source settings remain stored, but the firmware does not subscribe to room values, fetch forecasts or evaluate control proposals.",
    "dyn.strategy_help": "The shadow P controller calculates a whole-kelvin proposal at most every 30 minutes. It uses a 0.25 K deadband, limits the proposal to ±2 K and each step to 1 K. The proposal is evidence only and is not applied.",
    "dyn.strategy_off_help": "No control proposal is calculated while the firmware switch is off.",
    "dyn.safety_help": "Output is read-only. The firmware sends no control commands or setpoints to the heat pump or HomeHub.",
    ...
    "ref.max_age_help": "Maximum permitted age of the source reading, from 10 to 3600 seconds.",
    ...
    "ref.detail.status_label": "Status:", "ref.status.fresh": "Current",
    "ref.status.stale": "Stale", "ref.status.waiting": "Waiting",
    "ref.status.unavailable": "Unavailable", "ref.status.inactive": "Off",
    "ref.detail.fresh": "The latest MQTT sample passed mapping, plausibility and freshness checks.",
    "ref.detail.stale": "The latest sample is too old or its source time is not trustworthy; it is not used for a proposal.",
    "ref.detail.waiting": "No valid MQTT sample has been received yet.",
    "ref.detail.inactive": "Collection is disabled by the Firmware switch; the saved mapping remains available.",
    ...
    "wx.detail.inactive": "Forecast collection is disabled by the Firmware switch; the saved location remains available.",
    ...
    "wx.detail.source": "Open-Meteo · DWD ICON model. While the experimental feature is enabled, the ESP32 requests new data every 45 minutes. The forecast does not change heat-pump control.",
    ...
    "wx.hint": "Paste latitude and longitude copied from Google Maps into either field; the pair is split automatically. With the experimental Firmware switch enabled, the ESP32 requests temperature and solar radiation for this location from Open-Meteo every 45 minutes. Leave both fields empty to remove the source. Coordinates and the device's public IP are sent to Open-Meteo only while collection is enabled. No forecast changes heat-pump controls in Observe mode.",
    ...
    "ref.hint": "This stage supports one MQTT source. Save tests the mapping with a current MQTT message before storing it. The firmware subscribes and analyses values only while the experimental Firmware switch is enabled. Delete removes the source and captured sample.",
  ...
  de: {
    ...
    "dyn.card": "Dynamische Vorlaufregelung", "dyn.experimental": "Experimentell",
    "dyn.mode": "Betriebsart", "dyn.observe": "Beobachten", "dyn.off": "Aus",
    ...
    "dyn.enable_help": "Standardmäßig aus. Das Einschalten startet die experimentelle Schattenanalyse und ihre konfigurierten MQTT- und Wetterquellen. Erforderlich sind MQTT, eine vollständige Raumtemperaturquelle mit Sollwert und Zeitstempel sowie ein HomeHub. Der Schattenmodus schreibt niemals eine Korrektur an die Wärmepumpe.",
    "dyn.enabled": "Experimentelle Schattenanalyse eingeschaltet",
    "dyn.disabled": "Dynamische Vorlaufregelung ausgeschaltet",
    "dyn.enable_requirements": "Vor dem Einschalten MQTT, eine vollständige Raumtemperaturquelle und HomeHub konfigurieren.",
    "dyn.mode_help": "Beobachten abonniert den konfigurierten Raumeingang, ruft Wetterdaten ab und wertet den experimentellen Regler aus. Wärmepumpe und Vorlaufsollwert werden nicht verändert.",
    "dyn.off_help": "Die Funktion ist ausgeschaltet. Die Quellenkonfiguration bleibt gespeichert; die Firmware abonniert keine Raumwerte, ruft keine Prognosen ab und wertet keine Regelvorschläge aus.",
    "dyn.strategy_help": "Der Schatten-P-Regler berechnet höchstens alle 30 Minuten einen ganzzahligen Vorschlag. Er verwendet eine Totzone von 0,25 K, begrenzt den Vorschlag auf ±2 K und jeden Schritt auf 1 K. Der Vorschlag dient nur als Nachweis und wird nicht angewendet.",
    "dyn.strategy_off_help": "Solange der Firmware-Schalter aus ist, wird kein Regelvorschlag berechnet.",
    "dyn.safety_help": "Die Ausgabe ist nur lesend. Die Firmware sendet keine Steuerbefehle oder Sollwerte an Wärmepumpe oder HomeHub.",
    ...
    "ref.detail.status_label": "Status:", "ref.status.fresh": "Aktuell",
    "ref.status.stale": "Veraltet", "ref.status.waiting": "Wartet",
    "ref.status.unavailable": "Nicht verfügbar", "ref.status.inactive": "Aus",
    "ref.detail.fresh": "Der letzte MQTT-Messwert hat Zuordnungs-, Plausibilitäts- und Frischeprüfung bestanden.",
    "ref.detail.stale": "Der letzte Messwert ist zu alt oder seine Quellzeit nicht vertrauenswürdig; er wird für keinen Vorschlag verwendet.",
    "ref.detail.waiting": "Es wurde noch kein gültiger MQTT-Messwert empfangen.",
    "ref.detail.inactive": "Die Erfassung ist durch den Firmware-Schalter ausgeschaltet; die gespeicherte Zuordnung bleibt erhalten.",
    ...
    "wx.detail.inactive": "Die Prognoseerfassung ist durch den Firmware-Schalter ausgeschaltet; der gespeicherte Standort bleibt erhalten.",
    ...
    "wx.detail.source": "Open-Meteo · DWD-ICON-Modell. Bei eingeschalteter experimenteller Funktion fragt der ESP32 alle 45 Minuten neue Daten ab. Die Prognose verändert die Wärmepumpensteuerung nicht.",
    ...
    "wx.hint": "Aus Google Maps kopierten Breiten- und Längengrad in eines der beiden Felder einfügen; das Koordinatenpaar wird automatisch aufgeteilt. Bei eingeschaltetem experimentellem Firmware-Schalter fragt der ESP32 alle 45 Minuten Temperatur und Solarstrahlung für diesen Standort direkt bei Open-Meteo ab. Beide Felder leer lassen entfernt die Quelle. Koordinaten und öffentliche IP werden nur bei aktiver Erfassung an Open-Meteo übertragen. Im Beobachtungsmodus verändert keine Prognose die Wärmepumpensteuerung.",
    ...
    "ref.hint": "Diese Stufe unterstützt eine MQTT-Quelle. Speichern prüft die Zuordnung mit einer aktuellen MQTT-Nachricht, bevor sie übernommen wird. Die Firmware abonniert und analysiert Werte nur bei eingeschaltetem experimentellem Firmware-Schalter. Löschen entfernt Quelle und erfassten Messwert.",
    "syslog.title": "Syslog-Server", "syslog.hostport": "Host : Port",
