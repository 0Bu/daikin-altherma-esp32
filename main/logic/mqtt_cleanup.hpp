...
// 4 ENV III retained deletes, but esp-mqtt must own at most one of those QoS-1 tombstones at a
// time. The broker's PUBACK, not successful local queue admission, advances this state machine.
...
inline constexpr uint8_t MQTT_CLEANUP_ALL_SOURCES =
    ...
    static_cast<uint8_t>(MqttCleanupSource::Modbus) | static_cast<uint8_t>(MqttCleanupSource::Env3);
...
inline constexpr uint8_t MQTT_SOURCE_CLEANUP_MAX_STEPS = 1 + MQTT_WEATHER_CLEANUP_DISCOVERY_COUNT +
                                                         1 + MQTT_MODBUS_CLEANUP_DISCOVERY_COUNT +
                                                         1 + MQTT_ENV3_CLEANUP_DISCOVERY_COUNT;
...
template <size_t Capacity> class MqttCleanupEvidenceQueue {
public:
...
public:
    ...
    // same client retains its esp-mqtt outbox and therefore keeps the in-flight id without queuing
    // a duplicate. The epoch is assigned when a new client handle is built, not on CONNECTED.
    ...
    MqttCleanupAction next_action(bool connected, bool env3_enabled) noexcept {
        ...
        if (active_ == MqttCleanupSource::Env3 && !active_env3_enabled_)
            return {active_, MqttCleanupTopic::Env3Discovery, static_cast<uint8_t>(next_step_ - 1)};
        return {};
    }
    ...
    bool suppress(MqttCleanupSource source) const noexcept {
        ...
        return (pending_ & bit) != 0 || active_ == source || (completed_this_cycle_ & bit) != 0;
    }
...
private:
    MqttCleanupSource take_next_pending() noexcept {
        for (MqttCleanupSource source :
             {MqttCleanupSource::Weather, MqttCleanupSource::Modbus, MqttCleanupSource::Env3}) {
            const uint8_t bit = mqtt_cleanup_bit(source);
            if ((pending_ & bit) != 0) {
