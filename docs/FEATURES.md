...
| 42 | **24-hour trend rings** — a fixed-cadence (5 min × 288) `int16` ring per trended value or state, **static** rather than on the heap (576 B each; **23 X10A/board/state rings = 13248 B** plus **nine label-free HomeHub rings = 5184 B**, **18432 B total**, guarded by separate ceiling asserts). Their non-zero `pending` sentinel places the rings in `.data`, not `.bss`, so each also costs its size in the flash image. Chunked `GET /history` serves X10A by default and `source=modbus` for the second instrument; the UI draws the result in the value-row explainer and schematic inspector. Six numeric pairings overlay blue X10A and petrol HomeHub lines on one aligned scale. Four categorical timelines cover Smart-Grid mode, the tank heater, BUH and the 3-way valve: Boost marks mode 2; BSH pairs converter-qualified X10A BSH with HomeHub input 32 and latches any observed ON for the open bucket; BUH event-folds its converter-qualified Step1/Step2 bits independently and combines them into Aus/Stufe 1/Stufe 2; the valve pairs converter 306 with HomeHub input 37, retains the latest sampled position and names both DHW and space-circuit time. The UI calls these sampled raster intervals, not exact runtime. There is no heater-specific power register: HomeHub input 51 remains whole-system electrical input. Two X10A absences remain distinguished: a register that did not answer vs. an outdoor-unit page held over while the compressor rests. Five displayed figures (pump speed, ΔT, heat output, electrical input, COP) plus the combined BUH state have no single ring matching the shown figure; `www/js/history.js` assembles their curves/timeline from source rings using the live expressions. Two primary trends are the board's own **free heap** and **largest contiguous block**. Numeric row trends are addressed structurally by **(register page, byte offset, unit)**, shared-bit trends add the converter, and Smart-Grid combines two structurally identified contacts. HomeHub history membership is explicit in `HOMEHUB_HISTORIES`; compile-time guards pin it to real trends and unique offsets | ✅ 🧪 | [`logic/history.hpp`](../main/logic/history.hpp), [`logic/homehub_map.hpp`](../main/logic/homehub_map.hpp), [`history.cpp`](../main/history.cpp), [`http_status.cpp`](../main/http_status.cpp), [`www/js/history.js`](../main/www/js/history.js) |
