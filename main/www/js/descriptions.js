...
const DESCRIPTIONS = [
  ...
  { re: /after buh|outlet water buh|after buffer|tvbh/i,
    what: "Water temperature after the electric backup heater, usually measured by sensor R2T. It is downstream of the heat pump's own heat exchanger and includes any temperature rise added by the backup heater.",
    normal: "close to the pre-heater temperature while the backup heater is off; higher while the heater is adding heat. The water may then be routed to space heating or the tank.",
    de: { what: "Wassertemperatur nach dem elektrischen Zusatzheizer, üblicherweise gemessen von Fühler R2T. Sie liegt hinter dem Wärmetauscher der Wärmepumpe und enthält den Temperaturanstieg durch den Zusatzheizer.",
          normal: "bei Zusatzheizer OFF nahe an der Temperatur vor dem Heizer; höher, solange der Heizer Wärme ergänzt. Danach kann das Wasser zum Heizkreis oder zum Speicher geleitet werden." } },
  { re: /before buh|after phe|outlet water heat exch|leaving water.*\(?r1t\)?|tv inflow|outlet water heat exchanger/i,
    what: "Water temperature leaving the heat pump's heat exchanger before the electric backup heater, usually measured by sensor R1T. The UI uses it with return temperature and flow to estimate the heat pump's thermal output.",
    normal: "it should follow the active leaving-water target. The permitted range depends on the model and emitter type; Daikin's manuals specify different limits for underfloor heating, fan coils and radiators.",
    de: { what: "Wassertemperatur am Austritt des Wärmepumpen-Wärmetauschers vor dem elektrischen Zusatzheizer, üblicherweise gemessen von Fühler R1T. Die UI verwendet sie zusammen mit Rücklauftemperatur und Durchfluss, um die Wärmeleistung der Wärmepumpe zu schätzen.",
          normal: "sie sollte dem aktiven Vorlauf-Sollwert folgen. Der zulässige Bereich hängt von Modell und Heizflächenart ab; Daikin nennt unterschiedliche Grenzen für Fußbodenheizung, Gebläsekonvektoren und Heizkörper." } },
  { re: /inlet water|return water|tr return/i,
    what: "Water returning from the active hydraulic circuit into the unit, usually measured by sensor R4T. Leaving-water temperature minus this value is the current ΔT.",
    normal: "below the leaving-water temperature during normal heating. Compare the difference with the controller's target ΔT: Daikin varies it by model and emitter type, so 5 K is not a universal target.",
    de: { what: "Wasser, das aus dem aktiven Hydraulikkreis ins Gerät zurückkehrt, üblicherweise gemessen von Fühler R4T. Vorlauftemperatur minus diesen Wert ergibt das aktuelle ΔT.",
          normal: "im normalen Heizbetrieb unter der Vorlauftemperatur. Vergleiche die Differenz mit dem Ziel-ΔT des Reglers: Daikin legt es je nach Modell und Heizflächenart unterschiedlich fest; 5 K sind kein allgemeingültiges Ziel." } },
