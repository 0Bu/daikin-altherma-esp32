...
//
// legacy-194 opened this as "conv 114 x0.1 publishes 145.9-199.6 °C mid-run — physically impossible, but
...
// run-time wire bytes as the decisive experiment, and legacy-213 built logic/raw_capture.hpp to get them.
//
...
// string carries the 16-bit register EXACTLY — `raw = published * 10`, no information lost. legacy-194
...
// 54/54 is p ~ 1.6e-60. x0.01 (legacy-194's preferred candidate, chosen because 24.06 °C at rest "looked
...
// defect is therefore the legacy-35-legacy-39 shape exactly — a wrong converter ID on a right register — and
// not, as legacy-194 feared it might be, a wrong converter IMPLEMENTATION whose correction would move
...
inline constexpr ConvOverride CONV_OVERRIDES[] = {
    {0x10, 6, 114, 109,
     "legacy-194: 54/54 distinct wire integers satisfy raw==floor(128*T) on a 0.1 K grid (p~1.6e-60)"},
};
