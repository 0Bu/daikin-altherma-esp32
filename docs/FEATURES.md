...
- **✅ 🧪 Second SOURCE: read-only Modbus TCP to a Daikin HomeHub (EKRHH)**
  ...
  **no steady-state cost when absent** — Auto gets one bounded search window per boot and then retires
  the task; Off skips even that. This is what let the X10A poll task give its 4 KB back after an
  ...
  so the next cycle reconnects. **mDNS auto-discovery** is the default user mode: browse `_http._tcp`
  (what the hub advertises), accept up to 64 responders and filter the `homehub-*` hostname —
  mandatory because this firmware answers that same browse. Three attempts stop for the current boot;
  a later boot tries again, while only explicit Off suppresses future searches. Manual uses the typed
  address without browsing. A failed connect **backs off** through the X10A sweep's own policy.
