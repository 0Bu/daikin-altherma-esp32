...
// profile LABELS, which are C++ — so mirroring it here lets the CI logic-test gate it against the
// whole catalog (every detectable profile must select a real pre-BUH measurement, never a setpoint
...
// setpoint never do. A bare "heat exch" keyword is unusable: it also matches the
// outdoor/refrigerant rows ("O/U Heat Exch. Temp.(R4T)", "Outdoor heat exchanger temp.") and the
// DLWB2 hydro-split outlet — none of which is the leaving-water measurement. "mixed" is rejected so
// the EKMIK bizone mixed-zone R1T never wins over the main circuit's before-BUH R1T.
#include <cstddef>
...
inline bool lwt_is_water(const char* l) {
    return lwt_ci_contains(l, "leaving water") || lwt_ci_contains(l, "outlet water") ||
           lwt_ci_contains(l, "water heat exchanger outlet") || lwt_ci_contains(l, "inflow");
}
