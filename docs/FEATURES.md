...
| 62 | **Configurable MQTT room-temperature source — dynamic-LWT input slice.** Settings gives the whole staged project one permanent **Dynamic leaving-water control** card: **Observe** mode, room-temperature sources, weather forecast, control strategy, and read-only safety/output. The room-source and Open-Meteo location rows are editable today; the room source stores one presentation name, one exact MQTT topic (wildcards refused), one dot-separated numeric temperature path, an optional source-timestamp path and a 10–3600 s maximum age. A non-empty mapping is test-before-persist: one Save action validates the fields, calls `POST /test_ref_temp`, temporarily subscribes on the existing authenticated esp-mqtt client without changing Config/NVS, and presents the returned mapping-bound proof to `POST /set_ref_temp` only after the guarded MQTT task's ordinary JSON/timestamp/freshness path reads a real fresh value. Direct `/set_ref_temp` writes remain refused without that exact proof. The separate Test button is replaced by Delete: it posts the explicit empty mapping, removes the saved subscription, and resets the captured runtime value without applying unsaved draft fields. Activating an inactive popup text field selects its complete contents for one-paste replacement; later clicks in that active field preserve native caret placement. The live client reassembles a bounded payload outside the heap and extracts a finite number plus RFC3339/Unix-seconds source time. Freshness fails closed across reconnects: retained data is trusted only with source time, while a non-retained value without one ages from monotonic MQTT arrival. `/status.reference_temperature` reports both receive/source provenance, qualified age, freshness reason, retained flag and parse counters; the atomic config blob migrates v7 mappings to v8 with empty timestamp path and a 600 s limit. No history, averaging or heat-pump control consumes the value yet. The Shelly topic ending in `switch:0` plus path `temperature.tC` is a first-open UI transport-test preset, not a shipped default subscription; it measures Shelly device temperature, not room temperature. A Meross poller can map `meross/mts200b/<uuid>/state`, `thermostat.current_temperature_c`, and `thermostat.read_at` without firmware-specific vendor logic | ✅ 🧪 | [`mqtt_ha.cpp`](../main/mqtt_ha.cpp), [`logic/reference_temperature.hpp`](../main/logic/reference_temperature.hpp), [`http_config.cpp`](../main/http_config.cpp), [`www/js/dashboard.js`](../main/www/js/dashboard.js), [`www/js/settings.js`](../main/www/js/settings.js) |
